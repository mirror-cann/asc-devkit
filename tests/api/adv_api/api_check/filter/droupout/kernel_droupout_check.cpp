...
#include "impl/adv_api/detail/api_check/kernel_check/filter/dropout/dropout_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class DropOutAPICheck : public testing::Test {
protected:
...
TEST_F(DropOutAPICheck, DropOutAPICheckTestSuccess)
{
    ...
    m_pipe.InitBuffer(m_queMask, 1, 32 * sizeof(uint8_t));
    AscendC::LocalTensor<uint8_t> maskLocal = m_queMask.AllocTensor<uint8_t>();
    maskLocal.SetSize(32);

    AscendC::DropOutShapeInfo shapeInfo;
    shapeInfo.firstAxis = 4;
    shapeInfo.srcLastAxis = 8;
    shapeInfo.maskLastAxis = 1;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        DropOut, (float, false, 3), (dstTensor, srcTensor, maskLocal, sharedTmpBuffer, 0.5, shapeInfo));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(DropOutAPICheck, DropOutAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    shapeInfo.firstAxis = 4;
    shapeInfo.srcLastAxis = 8;
    shapeInfo.maskLastAxis = 0;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        DropOut, (uint8_t, false, 6), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, 2, shapeInfo));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 7);
}

TEST_F(DropOutAPICheck, DropOutAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
