...
#include "impl/adv_api/detail/api_check/kernel_check/pad/pad/unpad_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class UnPadAPICheck : public testing::Test {
protected:
...
TEST_F(UnPadAPICheck, UnPadAPICheckTestSuccess)
{
    ...
    unPadParams.leftPad = 0;
    unPadParams.rightPad = 1;
    UnPadTiling tiling;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(UnPad, (float), (dstTensor, srcTensor, unPadParams, sharedTmpBuffer, tiling));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(UnPadAPICheck, UnPadAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    AscendC::UnPadParams unPadParams;
    unPadParams.leftPad = 1;
    unPadParams.rightPad = 1;
    UnPadTiling tiling;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        UnPad, (uint8_t), (dstTensor[1], dstTensor[1], unPadParams, sharedTmpBuffer, tiling));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 3);
}

TEST_F(UnPadAPICheck, UnPadAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
    ...
    AscendC::UnPadParams unPadParams;
    unPadParams.leftPad = 26;
    unPadParams.rightPad = 33;
    UnPadTiling tiling;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        UnPad, (uint8_t), (dstTensor[1], dstTensor[1], unPadParams, sharedTmpBuffer, tiling));
