...
#include "impl/adv_api/detail/api_check/kernel_check/activation/geglu/geglu_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class GeGLUAPICheck : public testing::Test {
protected:
...
TEST_F(GeGLUAPICheck, GeGLUAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        GeGLU, (float, false), (dstTensor, srcTensor0, srcTensor1, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(GeGLUAPICheck, GeGLUAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        GeGLU, (uint8_t, false), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 11);
}

TEST_F(GeGLUAPICheck, GeGLUAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        GeGLU, (uint8_t, true), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
