...
#include "impl/adv_api/detail/api_check/kernel_check/activation/softmax/softmax_grad/softmax_grad_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class SoftMaxGradAPICheck : public testing::Test {
protected:
...
TEST_F(SoftMaxGradAPICheck, SoftMaxGradAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        ...
        (dstTensor, gradTensor, srcTensor, sharedTmpBuffer, tiling, false, softmaxShapeInfo));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(SoftMaxGradAPICheck, SoftMaxGradAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        SoftmaxGrad, (uint8_t, false, false),
...
TEST_F(SoftMaxGradAPICheck, SoftMaxGradAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
    ...
    SoftMaxTiling tiling;
    AscendC::SoftMaxShapeInfo softmaxShapeInfo = {13, 9, 1, 8};
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        SoftmaxGrad, (uint8_t, true, true),
        (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, tiling, false, softmaxShapeInfo));
