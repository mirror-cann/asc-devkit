...
#include "impl/adv_api/detail/api_check/kernel_check/activation/softmax/adjust_softmax_res/adjust_softmax_res_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class AdjustSoftMaxResAPICheck : public testing::Test {
protected:
...
TEST_F(AdjustSoftMaxResAPICheck, AdjustSoftMaxResAPICheckTestSuccess)
{
    ...
    AscendC::LocalTensor<float> softMaxRes = m_queDst.AllocTensor<float>();
    softMaxRes.SetSize(32);

    AscendC::TQue<AscendC::TPosition::VECIN, 1> m_queSrc;
    m_pipe.InitBuffer(m_queSrc, 1, 32 * sizeof(float));
    AscendC::LocalTensor<float> maxTensor = m_queSrc.AllocTensor<float>();
...
TEST_F(AdjustSoftMaxResAPICheck, AdjustSoftMaxResAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
...
TEST_F(AdjustSoftMaxResAPICheck, AdjustSoftMaxResAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
