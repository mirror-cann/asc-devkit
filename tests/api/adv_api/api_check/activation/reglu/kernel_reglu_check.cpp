...
TEST_F(ReGluAPICheck, ReGluAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        ReGlu, (float, false), (dstTensor, srcTensor0, srcTensor1, sharedTmpBuffer, calCount));
...
TEST_F(ReGluAPICheck, ReGluAPICheckTestSubFailure)
{
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        ReGlu, (uint8_t, false), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
...
TEST_F(ReGluAPICheck, ReGluAPICheckTestFailure)
{
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        ReGlu, (uint8_t, true), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
