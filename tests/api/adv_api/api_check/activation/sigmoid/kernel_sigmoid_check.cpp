...
#include "impl/adv_api/detail/api_check/kernel_check/activation/sigmoid/sigmoid_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class SigmoidAPICheck : public testing::Test {
protected:
...
TEST_F(SigmoidAPICheck, SigmoidAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Sigmoid, (float, false), (dstTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(SigmoidAPICheck, SigmoidAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        Sigmoid, (uint8_t, false), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 7);
}

TEST_F(SigmoidAPICheck, SigmoidAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        Sigmoid, (uint8_t, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
