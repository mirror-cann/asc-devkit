...
#include "impl/adv_api/detail/api_check/kernel_check/activation/gelu/faster_geluv2_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class FasterGeluV2APICheck : public testing::Test {
protected:
...
TEST_F(FasterGeluV2APICheck, FasterGeluV2APICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        FasterGeluV2, (float, false, true), (dstLocal, srcLocal, sharedTmpBuffer, dataSize));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(FasterGeluV2APICheck, FasterGeluV2APICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        FasterGeluV2, (uint8_t, true, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, dataSize));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 3);
}

TEST_F(FasterGeluV2APICheck, FasterGeluV2APICheckTestFailure)
{
    AscendC::TPipe m_pipe;
