...
#include "impl/adv_api/detail/api_check/kernel_check/activation/gelu/faster_gelu_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class FasterGeluAPICheck : public testing::Test {
protected:
...
TEST_F(FasterGeluAPICheck, FasterGeluAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        FasterGelu, (float, false, true), (dstLocal, srcLocal, sharedTmpBuffer, dataSize));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(FasterGeluAPICheck, FasterGeluAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        FasterGelu, (uint8_t, true, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, dataSize));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 3);
}

TEST_F(FasterGeluAPICheck, FasterGeluAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
