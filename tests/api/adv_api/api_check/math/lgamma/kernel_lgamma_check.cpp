...
#include "impl/adv_api/detail/api_check/kernel_check/math/lgamma/lgamma_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class LgammaAPICheck : public testing::Test {
protected:
...
TEST_F(LgammaAPICheck, LgammaAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Lgamma, (float, false), (dstTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(LgammaAPICheck, LgammaAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 32;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Lgamma, (uint8_t, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 7);
}

TEST_F(LgammaAPICheck, LgammaAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 32;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Lgamma, (half, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
