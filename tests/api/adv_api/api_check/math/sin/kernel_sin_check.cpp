...
#include "impl/adv_api/detail/api_check/kernel_check/math/sin/sin_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class SinAPICheck : public testing::Test {
protected:
...
TEST_F(SinAPICheck, SinAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Sin, (float, false), (dstTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(SinAPICheck, SinAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 32;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Sin, (uint8_t, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 5);
}

TEST_F(SinAPICheck, SinAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
