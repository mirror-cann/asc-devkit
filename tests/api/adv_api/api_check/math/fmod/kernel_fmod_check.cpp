...
#include "impl/adv_api/detail/api_check/kernel_check/math/fmod/fmod_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class FmodAPICheck : public testing::Test {
protected:
...
TEST_F(FmodAPICheck, FmodAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        Fmod, (float, false), (dstTensor, srcTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(FmodAPICheck, FmodAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        Fmod, (uint8_t, true), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 11);
}

TEST_F(FmodAPICheck, FmodAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
