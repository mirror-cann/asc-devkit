...
#include "impl/adv_api/detail/api_check/kernel_check/math/round/round_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class RoundAPICheck : public testing::Test {
protected:
...
TEST_F(RoundAPICheck, RoundAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Round, (float, false), (dstTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(RoundAPICheck, RoundAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 32;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Round, (uint8_t, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 5);
}

TEST_F(RoundAPICheck, RoundAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
