...
#include "impl/adv_api/detail/api_check/kernel_check/math/floor/floor_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class FloorAPICheck : public testing::Test {
protected:
...
TEST_F(FloorAPICheck, FloorAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Floor, (float, false), (dstTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(FloorAPICheck, FloorAPICheckTestNoTmpBufSuccess)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 8;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Floor, (float, false), (dstTensor, srcTensor, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(FloorAPICheck, FloorAPICheckTestNoTmpBufFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 32;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Floor, (uint8_t, true), (dstTensor[1], dstTensor[1], calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 7);
}

TEST_F(FloorAPICheck, FloorAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint8_t calCount = 32;
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(Floor, (uint8_t, true), (dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
