...
#include "impl/adv_api/detail/api_check/kernel_check/math/xor/xor_check.h"
#endif // ASCENDC_CPU_DEBUG
#include "impl/adv_api/detail/api_check/kernel_api_check.h"

class XorAPICheck : public testing::Test {
protected:
...
TEST_F(XorAPICheck, XorAPICheckTestSuccess)
{
    AscendC::TPipe m_pipe;

    AscendC::TQue<AscendC::TPosition::VECIN, 1> m_queDst;
    m_pipe.InitBuffer(m_queDst, 1, 32 * sizeof(int16_t));
    AscendC::LocalTensor<int16_t> dstTensor = m_queDst.AllocTensor<int16_t>();
    dstTensor.SetSize(32);

    AscendC::TQue<AscendC::TPosition::VECIN, 1> m_queSrc;
    m_pipe.InitBuffer(m_queSrc, 1, 32 * sizeof(int16_t));
    AscendC::LocalTensor<int16_t> srcTensor = m_queSrc.AllocTensor<int16_t>();
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        Xor, (int16_t, false), (dstTensor, srcTensor, srcTensor, sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 0);
}

TEST_F(XorAPICheck, XorAPICheckTestSubFailure)
{
    AscendC::TPipe m_pipe;
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(
        Xor, (uint8_t, true), (dstTensor[1], dstTensor[1], dstTensor[1], sharedTmpBuffer, calCount));
    EXPECT_EQ(AscendC::KernelRaise::GetInstance().GetRaiseCount() - startCounts, 11);
}

TEST_F(XorAPICheck, XorAPICheckTestFailure)
{
    AscendC::TPipe m_pipe;
