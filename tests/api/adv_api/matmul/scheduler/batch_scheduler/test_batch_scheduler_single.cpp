...
public:
    ...
    using MatmulTensorInfoA = CustomMatmulTensorInfo<IMPL, MM_CFG, MatmulInputAType<A_TYPE, typename A_TYPE::T>>;
...
class MatmulImpl : MATMUL_IMPORT_MODULE(BatchScheduler),
    ...
    MATMUL_ALLOW_USING(MatmulTensorInfoA);
    MATMUL_ALLOW_USING(MatmulTensorInfoB);
