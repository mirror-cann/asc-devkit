    ...
    uint32_t firstAxis, uint32_t srcLastAxis, uint32_t maskLastAxis, bool reverse, TestMode testMode)
{
    ...
    if (!reverse) {
        Select<T, U, reuse>(outputLocal, inputLocal, static_cast<T>(100), selMaskLocal, tmpLocal, info);
    } else {
        Select<T, U, reuse>(outputLocal, static_cast<T>(100), inputLocal, selMaskLocal, tmpLocal, info);
    }
...
#define VEC_SELECT_LEVEL2_TESTCASE(DATA_TYPE, MASK_TYPE, SRC_AXIS, MASK_AXIS, REVERSE, REUSE, testMode)   \
    TEST_F(SelectTest, Select##DATA_TYPE##MASK_TYPE##SRC_AXIS##MASK_AXIS##REVERSE##REUSE##testMode##Case) \
    {                                                                                                     \
        ...
        uint8_t outputGm[dataSize * sizeof(DATA_TYPE)] = {0};                                             \
                                                                                                          \
        MainVecSelectDemo<DATA_TYPE, MASK_TYPE, REUSE>(                                                   \
