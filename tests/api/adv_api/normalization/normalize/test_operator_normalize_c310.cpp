    ...
    GM_ADDR outputRstdGm, uint32_t aLength, uint32_t rLength, uint32_t rWithPad)
{
    ...
    for (int32_t i = 0; i < aLength; i++) {
        DataCopyPad(
            inputXLocal[i * rWithPad], inputXGlobal[i * rWithPad],
            {1, static_cast<uint16_t>(rWithPad * sizeof(T)), 0, 0}, dataCopyPadParams);
    }

    DataCopyPad(
        inputMeanLocal, inputMeanGlobal, {1, static_cast<uint16_t>(aLength * sizeof(float)), 0, 0}, dataCopyPadParams);
    DataCopyPad(
        inputVarLocal, inputVarGlobal, {1, static_cast<uint16_t>(aLength * sizeof(float)), 0, 0}, dataCopyPadParams);
    ...
    for (int32_t i = 0; i < aLength; i++) {
        DataCopyPad(
            outputGlobal[i * rWithPad], outputLocal[i * rWithPad],
            {1, static_cast<uint16_t>(rWithPad * sizeof(T)), 0, 0});
    }
    DataCopyPad(outputRstdGlobal, outputRstdLocal, {1, static_cast<uint16_t>(aLength * sizeof(T)), 0, 0});
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        ...
        // dtype combination
        normalizeTestParams{1, 32, 32, sizeof(float), sizeof(float), MainNormalizeTest<float, float>},
        normalizeTestParams{1, 32, 32, sizeof(half), sizeof(half), MainNormalizeTest<half, half>},
        normalizeTestParams{1, 32, 32, sizeof(half), sizeof(float), MainNormalizeTest<half, float>},
        // R != RwithPad
