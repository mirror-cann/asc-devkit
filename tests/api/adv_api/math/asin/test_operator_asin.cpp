    ...
    __gm__ uint8_t* __restrict__ dstGm, __gm__ uint8_t* __restrict__ srcGm, uint32_t dataSize, TestMode testMode)
{
    ...
    if (testMode == LEVEL0_COUNT_MODE) {
        ...
        Asin<T, false>(outputLocal, inputLocal, tmpLocal);
    }
...
VEC_ASIN_TESTCASE(half, LEVEL0_COUNT_MODE);
VEC_ASIN_TESTCASE(float, FIXED_BLOCK_MODE);
VEC_ASIN_TESTCASE(half, FIXED_BLOCK_MODE);
VEC_ASIN_TESTCASE(float, EXTRA_BUFFER_MODE);
VEC_ASIN_TESTCASE(half, EXTRA_BUFFER_MODE);