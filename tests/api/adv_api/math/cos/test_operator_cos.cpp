...
class TEST_COS : public testing::Test {
protected:
    ...
    __gm__ uint8_t* __restrict__ dstGm, __gm__ uint8_t* __restrict__ srcGm, uint32_t dataSize, TestMode testMode)
{
    ...
    if (testMode == NORMAL_MODE) {
        Cos<T, false>(outputLocal, inputLocal, dataSize);
        ...
        Cos<T, false>(outputLocal, inputLocal, tmpLocal);
    }
...
#define VEC_COS_LEVEL2_TESTCASE(DATA_TYPE, TEST_MODE)                            \
    TEST_F(TEST_COS, Cos##DATA_TYPE##TEST_MODE##Case)                            \
    {                                                                            \
        uint32_t dataSize = 256;                                                 \
        uint8_t inputGm[dataSize * sizeof(DATA_TYPE)];                           \
        uint8_t outputGm[dataSize * sizeof(DATA_TYPE)];                          \
                                                                                 \
        MainVecCosLevel2Demo<DATA_TYPE>(outputGm, inputGm, dataSize, TEST_MODE); \
                                                                                 \
        for (uint32_t i = 0; i < dataSize; i++) {                                \
            EXPECT_EQ(outputGm[i], 0x00);                                        \
        }                                                                        \
    }
VEC_COS_LEVEL2_TESTCASE(float, NORMAL_MODE);
VEC_COS_LEVEL2_TESTCASE(float, BASIC_BLK_MODE);
VEC_COS_LEVEL2_TESTCASE(half, NORMAL_MODE);
VEC_COS_LEVEL2_TESTCASE(half, BASIC_BLK_MODE);
