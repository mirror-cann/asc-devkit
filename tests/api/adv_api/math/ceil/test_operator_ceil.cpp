    ...
    __gm__ uint8_t* __restrict__ dstGm, __gm__ uint8_t* __restrict__ srcGm, uint32_t dataSize, TestMode testMode)
{
    ...
    if (testMode == COUNT) {
        Ceil<T>(outputLocal, inputLocal, dataSize);
        ...
        Ceil<T>(outputLocal, inputLocal, tmpLocal, dataSize);
    }
...
#define VEC_CEIL_LEVEL2_TESTCASE(TYPE, testMode, SIZE)           \
    TEST_F(CeilTest, Math_Ceil_##TYPE##_##testMode##_##SIZE)     \
    {                                                            \
        uint8_t inputGm[(SIZE) * sizeof(TYPE)];                  \
        uint8_t outputGm[(SIZE) * sizeof(TYPE)];                 \
        MainCeilDemo<TYPE>(outputGm, inputGm, (SIZE), testMode); \
        for (uint32_t i = 0; i < (SIZE); i++) {                  \
            EXPECT_EQ(outputGm[i], 0x00);                        \
        }                                                        \
    }
VEC_CEIL_LEVEL2_TESTCASE(float, COUNT, 256);
VEC_CEIL_LEVEL2_TESTCASE(float, COUNT_TMPBUF, 256);

VEC_CEIL_LEVEL2_TESTCASE(half, COUNT, 256);
VEC_CEIL_LEVEL2_TESTCASE(half, COUNT_TMPBUF, 256);
