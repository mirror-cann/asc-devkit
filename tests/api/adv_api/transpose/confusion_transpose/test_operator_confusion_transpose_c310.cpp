    ...
    const ShapeInfo srcShape, const uint32_t stackBufferSize, const uint32_t typeSize, ConfusionTransposeTiling& tiling)
{
    ...
    uint32_t dim0 = 1;
    uint32_t dim1 = srcShape.shape[0];
    uint32_t dim2 = srcShape.shape[1];
    if (shapeDims == 3) {
        dim0 = srcShape.shape[0];
    ...
    const ShapeInfo srcShape, const uint32_t stackBufferSize, const uint32_t typeSize, ConfusionTransposeTiling& tiling)
{
    ...
    uint32_t dim1 = srcShape.shape[1];
    uint32_t dim2 = srcShape.shape[2];
    tiling.param0 = dim0;
    tiling.param1 = dim1;
    tiling.param2 = dim2;
}
    ...
    const TransposeType transposeTypeIn, ConfusionTransposeTiling& tiling)
{
    ...
    } else if (transposeTypeIn == TransposeType::TRANSPOSE_ND2ND_102) {
        GetConfusionTranspose102TilingInfo(srcShape, stackBufferSize, typeSize, tiling);
    } else if (transposeTypeIn == TransposeType::TRANSPOSE_ND2ND_210) {
...
private:
    __aicore__ inline void CopyIn()
    {
        ...
        DataCopyPad(srcLocal, srcGlobal, copyParams, padParams);
    ...
    __aicore__ inline void Compute()
    {
        ...
        if constexpr (dim0Extent == 0) {
            ...
            srcLocal.SetShapeInfo(srcShapeInfo);

            uint32_t dstOriShape[] = {dim2, dim1};
            uint32_t dstShape[] = {dim2, dim1};
            ShapeInfo dstShapeInfo(2, dstShape, 2, dstOriShape, DataFormat::ND);
            dstLocal.SetShapeInfo(dstShapeInfo);

            LocalTensor<T> stackBuffer;
            PopStackBuffer<T, TPosition::LCM>(stackBuffer);
            uint32_t popSize = stackBuffer.GetSize();
            ConfusionTransposeTiling tiling;
            GetConfusionTransposeTilingInfo(srcShapeInfo, popSize, sizeof(T), transposetype, tiling);
            AscendC::ConfusionTranspose(dstLocal, srcLocal, transposetype, tiling);

            inQueueSrcVecOut.EnQue<T>(dstLocal);
            inQueueSrcVecIn.FreeTensor(srcLocal);

        } else {
            uint32_t srcOriShape[] = {dim0, dim1, dim2};
            uint32_t srcShape[] = {dim0, dim1, dim2};
            ShapeInfo srcShapeInfo(3, srcShape, 3, srcOriShape, DataFormat::ND);
            srcLocal.SetShapeInfo(srcShapeInfo);

            uint32_t dstOriShape[] = {dim0, dim2, dim1};
            uint32_t dstShape[] = {dim0, dim2, dim1};
    ...
    __aicore__ inline void CopyOut()
    {
        ...
        DataCopyExtParams copyParams;
        copyParams.blockCount = 1;
        copyParams.blockLen = static_cast<uint32_t>(dstExtent * sizeof(T));
        copyParams.srcStride = 0;
        copyParams.dstStride = 0;
...
__aicore__ inline void MainConfusionTranposeTest(GM_ADDR srcGm, GM_ADDR dstGm)
{
    ...
    op.Init(srcGm, dstGm, dim0, dim1, dim2, static_cast<TransposeType>(tranModeIn));
    op.Process();
}
