...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        if constexpr (!isReuse) {
            ...
            if constexpr (isAr) {
                ReduceMin<T, Pattern::Reduce::AR, isReuse>(dstLocal, srcLocal, tmp, shape, true);
            } else {
                ReduceMin<T, Pattern::Reduce::RA, isReuse>(dstLocal, srcLocal, tmp, shape, true);
            }
        } else {
            if constexpr (isAr) {
                ReduceMin<T, Pattern::Reduce::AR, isReuse>(dstLocal, srcLocal, shape, true);
            } else {
                ReduceMin<T, Pattern::Reduce::RA, isReuse>(dstLocal, srcLocal, shape, true);
            }
        }
...
__global__ __aicore__ void MainMin(
    __gm__ uint8_t* dstGm, __gm__ uint8_t* srcGm, uint32_t first, uint32_t last, bool isAr)
{
    if (isAr) {
        AscendC::KernelReduceMin<T, isReuse, true> op;
        op.Init(srcGm, dstGm, first, last);
        op.Process();
    } else {
        AscendC::KernelReduceMin<T, isReuse, false> op;
        op.Init(srcGm, dstGm, first, last);
        op.Process();
    }
}

struct MinTestParams {
    int32_t typeSize;
    int32_t first;
    int32_t last;
    bool isAr;
...
class MinTestsuite : public testing::Test, public testing::WithParamInterface<MinTestParams> {
protected:
...
INSTANTIATE_TEST_CASE_P(
    TEST_OPERATTION_MIN, MinTestsuite,
    ::testing::Values(
        MinTestParams{4, 8, 32, true, MainMin<float, true>}, MinTestParams{4, 7, 35, true, MainMin<float, true>},
        MinTestParams{4, 16, 128, true, MainMin<float, false>}, MinTestParams{4, 1, 8, false, MainMin<float, true>},
        MinTestParams{4, 8, 1, false, MainMin<float, false>}, MinTestParams{4, 16, 7, false, MainMin<float, true>},
        MinTestParams{4, 4, 8, true, MainMin<float, false>}, MinTestParams{4, 7, 1, true, MainMin<float, false>},
        MinTestParams{4, 4, 64, true, MainMin<float, true>}, MinTestParams{2, 16, 128, true, MainMin<float, false>},
        MinTestParams{2, 8, 16, true, MainMin<float, false>}, MinTestParams{2, 7, 55, true, MainMin<float, true>},
        MinTestParams{2, 7, 1, true, MainMin<float, false>}, MinTestParams{2, 1, 8, false, MainMin<float, true>},
        MinTestParams{2, 8, 1, false, MainMin<float, false>}, MinTestParams{2, 16, 15, false, MainMin<float, true>}));

TEST_P(MinTestsuite, MinOpTestCase)
{
    auto param = GetParam();
