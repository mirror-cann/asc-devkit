...
#define KERNEL_FIXPIPE(fmT, wT, l1outT, dstT, fmTSize, wTSize, l1outTSize, dstTSize, name, deqMode, fixpipeFunc)      \
    ...
    {                                                                                                                 \
        ...
        if (enNz2nd) {                                                                                                \
            fixpipeParams = {coutBlocks, static_cast<uint16_t>(howo * 16 * l1outTSize / 32), 0, inputParams.cout};    \
        ...
        if (fixpipeParams.quantParams.quantPre == QuantMode_t::VDEQF16 ||                                             \
            ...
            Fixpipe(dst_l1, dst_l0c, cbufWorkspace, fixpipeParams);                                                   \
            Fixpipe(dst_ub, dst_l0c, cbufWorkspace, fixpipeParams);                                                   \
            Fixpipe(outputGm, dst_l0c, cbufWorkspace, fixpipeParams);                                                 \
        } else {                                                                                                      \
            Fixpipe(dst_l1, dst_l0c, fixpipeParams);                                                                  \
            Fixpipe(dst_ub, dst_l0c, fixpipeParams);                                                                  \
...
KERNEL_FIXPIPE(half, half, float, half, 2, 2, 4, 2, f322bf16_relu, QuantMode_t::F322BF16, FIXPIPE_DEQ_CONV_RELU_FUNC)
KERNEL_FIXPIPE(
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        ...
        FixpipeTestParams{{2, 4, 4, 2, 2, 128, 16, 2, 2}, 2, 2, 4, 2, kernel_fixpipe_f322bf16_relu, true, true},
