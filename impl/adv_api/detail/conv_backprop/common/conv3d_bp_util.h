...
static __aicore__ inline uint32_t CalRows2Copy(uint32_t copySize, uint32_t width)
{
    ...
    if (copySize > width) {
        ...
        if (copySize == rows * width) {
            return rows; // Integer division returns directly. When not integer division, Ceil defaults to moving one
                         // more line
        } else if ((2 * copySize) % width != 0) {
            return rows + 1; // Unless the tail block is 0.5 lines, you will need to move one more line when trailing up
                             // and down
        }
        return rows;
        ...
        return rows; // When the width is an integer multiple of size, there is no trailing, just move it to one line,
                     // otherwise move it to two lines
    }
}

// The default overloaded function of the call function defined in the API class supports any number of parameters of
// any type
...
Line 49: decltype gets the type of expression. declval is a template function that gets the rvalue reference of template
parameter T. If T does not have MEMBER<U>, an error will be reported, otherwise TrueType will be returned
*/
    ...
    {                                                                                                           \
        ...
        struct _has_impl_##MEMBER {                                                                             \
            ...
            static auto check(int)                                                                              \
                -> decltype(std::declval<typename C::template MEMBER<U, ##args>>(), AscendC::TrueType());       \
            template <typename C>                                                                               \
...
#define DECLARE_CHECK_SYNC_IMPL(NAMESPACE, MEMBER, args...)                                                     \
    ...
    {                                                                                                           \
        ...
        struct _has_impl_##MEMBER {                                                                             \
            template <typename C>                                                                               \
            static auto check(int)                                                                              \
                -> decltype(std::declval<typename C::template MEMBER<U, sync, ##args>>(), AscendC::TrueType()); \
            template <typename C>                                                                               \
            static AscendC::FalseType check(...);                                                               \
            static constexpr bool value = AscendC::IsSameType<decltype(check<T>(0)), AscendC::TrueType>::value; \
        };                                                                                                      \
...
// Define the member function MEMBER<U>. If the MEMBER member exists in Config, the MEMBER function points to the Config
// member, otherwise it points to Current::Init
...
// Define the member function MEMBER<U, sync>. If the MEMBER member exists in Config, the MEMBER function points to the
// Config member, otherwise it points to Current::Init
...
At the same time, a verification template function is defined. The function returns the corresponding template class
based on whether the template parameter T has a constant and a member M with a value > 0. Used with macro
DEFINE_STUCTï¼?
*/
