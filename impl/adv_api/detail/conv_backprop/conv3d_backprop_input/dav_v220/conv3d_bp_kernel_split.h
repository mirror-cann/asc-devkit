    ...
    AscendC::LocalTensor<typename Intf::L0cT>& useC1Buf, QuantMode_t fixPipeQuantMode)
{
    ...
    if (self->ctx.splitWi_ % AscendC::BLOCK_CUBE) {
        fixpipeParams.nSize = AscendC::BLOCK_CUBE;
        ...
        for (uint32_t nIdx = 0; nIdx < Ceil(self->ctx.baseUseN_, AscendC::BLOCK_CUBE); ++nIdx) {
            ...
            for (uint32_t hIdx = 0; hIdx < Ceil(self->ctx.baseUseM_, AscendC::BLOCK_CUBE); ++hIdx) {
                Fixpipe(output[dstOffset], useC1Buf[srcOffset], fixpipeParams);
                srcOffset += (self->ctx.splitWi_ * AscendC::BLOCK_CUBE);
                dstOffset += (self->ctx.tiling_->strideH * self->ctx.tiling_->wi * AscendC::BLOCK_CUBE);
            }
        }
    } else {
        fixpipeParams.nSize = AscendC::BLOCK_CUBE;
        ...
        for (uint32_t nIdx = 0; nIdx < Ceil(self->ctx.baseUseN_, AscendC::BLOCK_CUBE); ++nIdx) {
            ...
            dstOffset = inOneComputeDstOffset + outOneComputeDstOffset;
            Fixpipe(output[dstOffset], useC1Buf[srcOffset], fixpipeParams);
            srcOffset += (self->ctx.baseUseM_ * AscendC::BLOCK_CUBE);
        }
    }
}
} // namespace ConvBackpropInputFunc
