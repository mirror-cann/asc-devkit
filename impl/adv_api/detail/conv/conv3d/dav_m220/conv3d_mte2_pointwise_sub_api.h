...
public:
    ...
    __aicore__ inline void LoadAL1()
    {
        ...
        KERNEL_LOG(
            KERNEL_DEBUG,
            "[LoadAL1WithPointWise] nd2NzParams.nValue %d nd2NzParams.dValue %d "
        ...
        if (dihiwi > ConvApi::MAX_UINT16) {
            ...
            for (uint64_t i = 0; i < currentKAL1; ++i) {
                AscendC::DataCopy<typename Intf::InputT>(
                    self_->ctx.al1[i * al1BlockNum], self_->ctx.agm[aL1GmOffset + i * dihiwi], nd2NzParams);
            }
        } else {
            SetNd2NzParams(currentKAL1, dihiwi, alignCurrentKAL1);
            AscendC::DataCopy<typename Intf::InputT>(self_->ctx.al1, self_->ctx.agm[aL1GmOffset], nd2NzParams);
        }
    }
    ...
    __aicore__ inline void loadAL1ByKAlignTo16()
    {
        ...
        for (uint64_t copyIdx = 0; copyIdx < upperLimits; ++copyIdx) {
            ...
            if (dihiwi > ConvApi::MAX_UINT16) {
                ...
                AscendC::DataCopy<typename Intf::InputT>(
                    self_->ctx.al1[singleDataCopyOffset], self_->ctx.agm[aL1GmOffset + dihiwi * currentDataCopyLines],
                    nd2NzParams);
            }
        }
    ...
    __aicore__ inline void LoadBL1()
    {
        ...
        if (self_->ctx.orgCi > ConvApi::MAX_UINT16) {
            ...
            for (uint64_t i = 0; i < currentNBL1; ++i) {
                AscendC::DataCopy<typename Intf::WeightT>(
                    self_->ctx.bl1[i * bl1BlockNum], self_->ctx.bgm[bL1GmOffset + self_->ctx.orgCi * i], nd2NzParams);
            }
        } else {
            KERNEL_LOG(
                KERNEL_DEBUG,
                "[LoadBL1WithPointWise] nd2NzParams.nValue %d "
                "nd2NzParams.dValue %d nd2NzParams.srcDValue %d nd2NzParams.dstNzC0Stride %d.\n",
                nd2NzParams.nValue, nd2NzParams.dValue, nd2NzParams.srcDValue, nd2NzParams.dstNzC0Stride);
...
private:
    ...
    __aicore__ inline void SetInitConstValueParams(
        AscendC::InitConstValueParams<typename Intf::WeightT>& initConstValueParams, const uint64_t repeatTimes,
        const uint64_t blockNum)
    {
        initConstValueParams.repeatTimes = repeatTimes;
        initConstValueParams.blockNum = blockNum;
