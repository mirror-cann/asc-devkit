...
#define CONV_DECLARE_REG_IMPL(MEMBER, args...) namespace __AuxCheckImpl {                                             \
    ...
    struct _has_impl_##MEMBER {                                                                                       \
        template<typename C>                                                                                          \
        static auto check(int) -> decltype(std::declval<typename C::template MEMBER<U, 0, ##args>>(), AscendC::TrueType());    \
        template<typename C>                                                                                          \
        static AscendC::FalseType check(...);                                                                                  \
        enum { value = AscendC::IsSameType<decltype(check<T>(0)), AscendC::TrueType>::value };                                          \
    };                                                                                                                \
}
...
#define CONV_CHECK_FUN(T, NAMESPACE, ... )                                                                            \
    (!AscendC::IsSameType<decltype(T::call(__VA_ARGS__)), NAMESPACE::TypeFalse>::value)
...
#define CONV_DECLARE_CHECK_MEMBER(MEMBER) namespace __AuxCheck {                                                      \
    ...
    struct _has_member_##MEMBER {                                                                                     \
        ...
        template<typename U>                                                                                          \
        static int check(...);                                                                                        \
        enum { value = AscendC::IsSameType<decltype(check<T>(0)), void>::value };                                              \
    };                                                                                                                \
}
