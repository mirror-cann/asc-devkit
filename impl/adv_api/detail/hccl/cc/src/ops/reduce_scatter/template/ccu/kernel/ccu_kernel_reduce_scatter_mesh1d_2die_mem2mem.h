...
public:
    explicit CcuKernelArgReduceScatterMeshMem2Mem1D2Die(
        uint64_t gRankSize, uint64_t rankSize, bool isReduceToOutput, uint32_t rankId, const OpParam& opParam,
        const std::vector<uint32_t>& subRankGroup, const std::vector<std::vector<uint32_t>>& subCommRanks)
    ...
    {
        HCCL_DEBUG(
            "[CcuKernelArgReduceScatterMeshMem2Mem1D2Die] dimSize: %lu, rankId: %u, reduceOp: %d, dataType: %d",
...
public:
    explicit CcuTaskArgReduceScatterMeshMem2Mem1D2Die(
        ...
        : inputAddr_(inputAddr),
          ...
          sliceSize_(lastSliceSize),
          repeatNum_(repeatNum)
    {
        HCCL_DEBUG(
            "[CcuTaskArgReduceScatterMeshMem2Mem1D2Die] inputAddr: %lu, outputAddr: %lu, scratchAddr: %lu, "
            "inputSliceStride: %lu, inputRepeatStride: %lu, outputRepeatStride: %lu, normalSliceSize: %lu, "
            "lastSliceSize: %lu, repeatNum: %lu",
            inputAddr_, outputAddr_, scratchAddr_, inputSliceStride_, inputRepeatStride_, outputRepeatStride_,
            normalSliceSize_, sliceSize_, repeatNum_);
    }
