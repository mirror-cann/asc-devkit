...
HcclResult CcuKernelReduceScatterMesh1D2DieMem2Mem::RmtReduce()
{
    ...
    CCU_WHILE(repeatNum_ != UINT64_MAX)
    {
        ...
        CCU_IF(sliceSize_ != 0) { CHK_RET(DoReduceScatter()); }
        flag_ = 1;
    }
    return HCCL_SUCCESS;
}
...
void CcuKernelReduceScatterMesh1D2DieMem2Mem::ReduceLoopGroup(
    CcuRep::LocalAddr outDstOrg, std::vector<CcuRep::LocalAddr>& scratchOrg)
{
...
std::vector<uint64_t> CcuKernelReduceScatterMesh1D2DieMem2Mem::GeneArgs(const CcuTaskArg& arg)
{
    ...
    std::vector<uint64_t> taskArgs = {
        myInput,           myOutput,           myToken,   myScratch, currentRankSliceInputOffset,
        inputRepeatStride, outputRepeatStride, sliceSize, repeatNum};
    ...
    HCCL_INFO(
        "[CcuKernelReduceScatterMesh1D2DieMem2Mem::GeneArgs]currentRankSliceInputOffset[%d], sliceSize[%d], "
        "rankSize_[%d], scratchBaseOffset0[%d], scratchBaseOffset1[%d]",
