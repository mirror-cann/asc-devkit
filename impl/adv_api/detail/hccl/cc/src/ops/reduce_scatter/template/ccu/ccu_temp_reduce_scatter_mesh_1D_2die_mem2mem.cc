    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    for (int dieId = 0; dieId < DIE_NUM; dieId++) {
        ...
        kernelInfo.kernelArg = std::make_shared<CcuKernelArgReduceScatterMeshMem2Mem1D2Die>(
            templateRankSize_, subRankGroup[dieId].size(), isReduceToOuput, myRank_, param, subRankGroup[dieId],
            subCommRanks_);
        HCCL_INFO(
            "[CcuTempReduceScatterMeshMem2Mem1D2Die]gRankSize[%d], rankSize[%d], myRank[%d], isReduceToOuput[%d]",
    ...
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    for (uint64_t dieIdx = 0; dieIdx < DIE_NUM; dieIdx++) {
        ...
        HcclCcuKernelLaunch(
            param.hcclComm, templateResource.threads[dieIdx], templateResource.ccuKernels[dieIdx], taskArgPtr);
    }
    // 后流同步
