...
CcuKernelReduceScatterNHR1DMem2Mem::CcuKernelReduceScatterNHR1DMem2Mem(const CcuKernelArg& arg) : CcuKernelAlgBase(arg)
{
    ...
    HCCL_INFO(
        "[CcuKernelReduceScatterNHR1DMem2Mem] KernelArg: mySubCommRankId_[%u], myRankIdx_[%d], axisId_[%u], "
        "dimSize_[%u], localSize_[%u], "
...
HcclResult CcuKernelReduceScatterNHR1DMem2Mem::InitResources()
{
    ...
    for (uint32_t channelIdx = 0; channelIdx < localSize_; channelIdx++) {
        HCCL_INFO(
            "[CcuKernelReduceScatterNHR1DMem2Mem] mySubCommRankId[%u], channelId[%u]", mySubCommRankId_, channelIdx);
...
void CcuKernelReduceScatterNHR1DMem2Mem::DoRepeatReduceScatterNHR()
{
    ...
    CCU_WHILE(repeatNumVar_ != UINT64_MAX)
    {
        ...
        event_.SetMask(1);
        CCU_IF(localSliceSize != 0) { LocalCopyNb(localDst_, localSrc_, localSliceSize, event_); }
        CCU_IF(localSliceSize == 0) { RecordEvent(event_); }
        WaitEvent(event_);
        isRepeatIter_ = 1;
    }
}

void CcuKernelReduceScatterNHR1DMem2Mem::DoRepeatReduceScatterNHRSingleStep(
...
void CcuKernelReduceScatterNHR1DMem2Mem::DoRepeatWriteReduceSlices(
    const u32& toRank, CcuRep::LocalAddr& src, CcuRep::RemoteAddr& dst, const bool islastSlice)
{
    ...
    CCU_WHILE(repeatNumVarTemp_ != UINT64_MAX)
    {
        ...
        event_.SetMask(1);
        CCU_IF(sliceSize_ != 0) { WriteReduceNb(sendChannel, dst, src, sliceSize_, dataType_, reduceOp_, event_); }
...
std::vector<uint64_t> CcuKernelReduceScatterNHR1DMem2Mem::GeneArgs(const CcuTaskArg& arg)
{
    ...
    HCCL_INFO(
        "[CcuKernelReduceScatterNHR1DMem2Mem] TaskArgs: inputAddr[%llu], outputAddr[%llu],"
        ...
        inputAddr, outputAddr, die0Size, die1Size, die0LastSliceSize, die1LastSliceSize, inputSliceStride,
