...
CcuKernelAllGatherMesh1D::CcuKernelAllGatherMesh1D(const CcuKernelArg& arg) : CcuKernelAlgBase(arg)
{
    const CcuKernelArgAllGatherMesh1D* kernelArg = dynamic_cast<const CcuKernelArgAllGatherMesh1D*>(&arg);
    rankId_ = kernelArg->rankId_;
    rankSize_ = kernelArg->dimSize_;
    channels_ = kernelArg->channels;

    HCCL_INFO("[CcuKernelAllGatherMesh1D] Init, KernelArgs are rankId[%u], rankSize_[%u]", rankId_, rankSize_);
}
...
HcclResult CcuKernelAllGatherMesh1D::InitResource()
{
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            HCCL_DEBUG("[CcuKernelAllGatherMesh1D] MyRank[%u], PeerId[%u], ChannelId[%u]", rankId_, peerId, channelIdx);
...
std::vector<uint64_t> CcuKernelAllGatherMesh1D::GeneArgs(const CcuTaskArg& arg)
{
    ...
    HCCL_INFO(
        "[CcuKernelAllGatherMesh1D] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
