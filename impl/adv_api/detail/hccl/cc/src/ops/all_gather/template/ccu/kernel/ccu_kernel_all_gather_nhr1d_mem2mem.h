...
public:
    explicit CcuKernelArgAllGatherNHR1D(
...
public:
    explicit CcuTaskArgAllGatherNHR1D(
        ...
        uint64_t outputRepeatStride, uint64_t isInputOutputEqual, uint64_t die0LastSize, uint64_t die1LastSize)
        : inputAddr_(inputAddr),
          ...
          die0LastSize_(die0LastSize),
          die1LastSize_(die1LastSize)
    {}
...
private:
    ...
    void DoRepeatSendRecvSlices(
        const u32& toRank, hcomm::CcuRep::LocalAddr& src, hcomm::CcuRep::RemoteAddr& dst, u32 signalIndex,
        bool islastSlice);
