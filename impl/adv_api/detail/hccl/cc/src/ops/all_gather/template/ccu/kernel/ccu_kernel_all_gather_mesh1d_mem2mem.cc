...
CcuKernelAllGatherMesh1DMem2Mem::CcuKernelAllGatherMesh1DMem2Mem(const CcuKernelArg& arg) : CcuKernelAlgBase(arg)
{
    const CcuKernelArgAllGatherMesh1DMem2Mem* kernelArg = dynamic_cast<const CcuKernelArgAllGatherMesh1DMem2Mem*>(&arg);
    rankId_ = kernelArg->rankId_;
    rankSize_ = kernelArg->dimSize_;
    channels_ = kernelArg->channels;

    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2Mem] Init, KernelArgs are rankId[%u], rankSize_[%u]", rankId_, rankSize_);
}
...
HcclResult CcuKernelAllGatherMesh1DMem2Mem::InitResource()
{
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            HCCL_DEBUG(
                "[CcuKernelAllGatherMesh1DMem2Mem] MyRank[%u], PeerId[%u], ChannelId[%u]", rankId_, peerId, channelIdx);
...
void CcuKernelAllGatherMesh1DMem2Mem::PostSync()
{
    for (ChannelHandle channel : channels_) {
        NotifyRecord(
            channel, CKE_IDX_0, 1 << POST_SYNC_ID); // bit index = 4, 用作后同步。cke都可以用同一个，所以都是CKE_IDX_0
    }
    for (ChannelHandle channel : channels_) {
...
void CcuKernelAllGatherMesh1DMem2Mem::DoAllGather(
    const hcomm::CcuRep::LocalAddr& src, const std::vector<hcomm::CcuRep::RemoteAddr>& dst,
...
{
    const CcuTaskArgAllGatherMesh1DMem2Mem* taskArg = dynamic_cast<const CcuTaskArgAllGatherMesh1DMem2Mem*>(&arg);
    ...
    HCCL_INFO(
        "[CcuKernelAllGatherMesh1DMem2Mem] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
