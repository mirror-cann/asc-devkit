...
public:
    explicit CcuKernelArgAllGatherMesh1D(
    ...
    {
        HCCL_DEBUG("[CcuKernelArgAllGatherMesh1D] dimSize: %lu, rankId: %u", dimSize_, rankId_);
    }
...
public:
    explicit CcuTaskArgAllGatherMesh1D(
        uint64_t inputAddr, uint64_t outputAddr, uint64_t token, uint64_t offset, uint64_t sliceSize)
        : inputAddr_(inputAddr), outputAddr_(outputAddr), token_(token), offset_(offset), sliceSize_(sliceSize)
    {
        HCCL_DEBUG(
            "[CcuTaskArgAllGatherMesh1D] inputAddr: %lu, outputAddr: %lu, offset: %lu, "
            "sliceSize: %lu",
