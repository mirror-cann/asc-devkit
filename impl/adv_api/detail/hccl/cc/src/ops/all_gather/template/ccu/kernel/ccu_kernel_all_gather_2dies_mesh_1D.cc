...
CcuKernelAllGather2DiesMesh1D::CcuKernelAllGather2DiesMesh1D(const hcomm::CcuKernelArg& arg) : CcuKernelAlgBase(arg)
{
    const CcuKernelArgAllGather2DiesMesh1D* kernelArg = dynamic_cast<const CcuKernelArgAllGather2DiesMesh1D*>(&arg);
    rankId_ = kernelArg->rankId_;    // 本rankId
    rankSize_ = kernelArg->dimSize_; // 未划分的全部的dimSize
    rankIdGroup_ =
        kernelArg->rankIdGroup_; // 划分后本kernel需要处理的rankIdGroup，序号存放需要是递增的，不包括自身rankId
...
{
    const CcuTaskArgAllGather2DiesMesh1D* taskArg = dynamic_cast<const CcuTaskArgAllGather2DiesMesh1D*>(&arg);
    ...
    std::vector<uint64_t> taskArgs = {inputAddr,      outputAddr,     sliceSize,      offSet,        token,
                                      groupOpSize[0], groupOpSize[1], groupOpSize[2], groupOpSize[3]};
    HCCL_INFO(
        "[CcuKernelAllGather2DiesMesh1D] TaskArgs: inputAddr[%llu], outputAddr[%llu], sliceSize[%llu], offSet[%llu]",
