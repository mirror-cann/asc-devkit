...
HcclResult InsTempAlltoAllVMesh1D::RunSendRecvByChannel(const TemplateDataParams &tempAlgParams,
    const u32 roundIdx, const std::vector<ChannelInfo> &curChannels, const u32 remoteRank,
    const std::vector<ThreadHandle> &threads, const u32 commLoops) const
{
    ...
    for (u32 channelId = 0; channelId < curChannels.size(); channelId++) {
        if (roundIdx != 0 && isDmaRead_ && sendSizeSplit_[channelId] > 0) {
            CHK_RET(PreCopy(tempAlgParams, threads[queIdx], myRankCclBuffIdx, remoteRank,
