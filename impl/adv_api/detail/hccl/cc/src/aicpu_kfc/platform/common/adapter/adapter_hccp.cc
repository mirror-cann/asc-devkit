    ...
    RdmaHandle rdmaHandle, int flag, int qpMode, struct TypicalQp* qpInfo, QpHandle& qpHandle)
{
    ...
    RPT_ENV_ERR(
        ...
        std::vector<std::string>({"qp", qpInfoStr}));
...
HcclResult hrtGetIfNum(struct RaGetIfattr& config, u32& num)
{
    ...
    CHK_PRT_RET(
        ret != 0,
        HCCL_ERROR(
            "[Get][IfNum]errNo[0x%016llx] ra get if num fail. ret[%d], num[%u]", HCCL_ERROR_CODE(HCCL_E_TCP_CONNECT),
            ret, num),
        HCCL_E_TCP_CONNECT);
    return HCCL_SUCCESS;
#else
...
{
    string qpInfo =
        string("rdmaHandle:[") + to_string(reinterpret_cast<intptr_t>(rdmaHandle)) + string("],qpHandle:[") +
        ...
        string(",sendCqCompVector:") + to_string(attrs->cqAttr.sendCqCompVector) + string(",recvCqCompVector:") +
...
HcclResult hrtRaRecvWrlist(QpHandle handle, struct RecvWrlistData* wr, unsigned int recvNum, unsigned int* completeNum)
{
    ...
    *completeNum = 0;
    while (true) {
        if (remainNum < 0) {
            HCCL_ERROR(
                "[Recv][RaWr]ra wr list Recv async fail. return[%d], remainNum[%u], "
                "recvNum[%u].",
