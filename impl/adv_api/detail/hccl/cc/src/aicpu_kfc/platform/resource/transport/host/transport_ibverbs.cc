...
HcclResult TransportIbverbs::ConstructExchangeForSend()
{
    ...
    if (machinePara_.isAicpuModeEn) {
        ...
        CHK_RET(
            CreateNotifyBuffer(dataAckNotify_, MemType::DATA_ACK_NOTIFY_MEM, exchangeDataPtr, exchangeDataBlankSize));
    }
    ...
    if (useAicpu || qpMode_ == QPMode::NORMAL) {
        ...
        if (isA3Aicpu && workFlowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE &&
            ...
            attrs.qpAttr.cap.max_send_wr =
                machinePara_.queueDepthAttr.sqDepth == INVALID_UINT ? AICPU_SQ_CQ_DEPTH : attrs.qpAttr.cap.max_send_wr;
            attrs.cqAttr.sendCqDepth =
                machinePara_.queueDepthAttr.sendCqDepth == INVALID_UINT ? AICPU_SQ_CQ_DEPTH : attrs.cqAttr.sendCqDepth;
        }
    ...
    NotifyLoadType notifyLoadType)
{
    ...
    if (machinePara_.isAicpuModeEn || (machinePara_.nicDeploy == NICDeployment::NIC_DEPLOYMENT_DEVICE &&
        ...
        CHK_RET(HrtRaGetNotifyBaseAddr(
            nicRdmaHandle_, &notifyBaseVa, &notifyTotalSize, [this]() -> bool { return this->GetStopFlag(); }));
        CHK_PRT_RET(
