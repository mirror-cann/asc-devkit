...
HcclResult ConstructQpAttrs(
    s32 qpMode, struct QpExtAttrs& attrs, const QueueDepthAttr& qpDepth, bool isWorkFlowLib = false);
...
HcclResult HrtRaQpConnectAsync(
    QpHandle handle, const SocketHandle sockHandle, std::function<bool()> needStop = []() { return false; },
    u32 timeout = 0);
...
HcclResult HrtRaGetNotifyBaseAddr(
    RdmaHandle handle, u64* va, u64* size, std::function<bool()> needStop = []() { return false; });
...
HcclResult hrtRaSocketBatchConnect(
    struct SocketConnectInfoT conn[], u32 num, u32 maxLen = MAX_VALUE_U32,
...
HcclResult hrtRaSocketBlockSend(
    const FdHandle fdHandle, const void* data, u64 sendSize, std::function<bool()> needStop = []() { return false; });
...
HcclResult hrtRaSocketBlockRecv(
    const FdHandle fdHandle, void* data, u64 size, std::function<bool()> needStop = []() { return false; },
    u32 timeout = 0);
