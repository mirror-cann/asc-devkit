...
TransportDeviceIbverbs::TransportDeviceIbverbs(
    ...
    : TransportIbverbs(dispatcher, notifyPool, machinePara, timeout), transDevIbverbsData_(transDevIbverbsData)
{}
    ...
    if (localAddr != nullptr) {
        ...
        if (dstAddr >= remoteInputAddr && dstAddr < remoteInputAddr + remoteInputSize) {
            ...
            HCCL_ERROR(
                "[TransportDeviceIbverbs][TxAsync]src_ptr=%p is out of range, inputmem src[%p], size[%llu];"
                " outputmem src[%p] size[%llu]",
                remoteAddr, remoteInputAddr, remoteInputSize, remoteOutputAddr, remoteOutputSize);
            return HCCL_E_INTERNAL;
        }
        ...
        if (srcAddr >= localInputMem_.addr && srcAddr < localInputMem_.addr + localInputMem_.size) {
            ...
            HCCL_ERROR(
                "[TransportDeviceIbverbs][TxAsync]src_ptr=%p is out of range, inputmem src[%p], size[%llu];"
                " outputmem src[%p] size[%llu]",
                localAddr, localInputMem_.addr, localInputMem_.size, localOutputMem_.addr, localOutputMem_.size);
            return HCCL_E_INTERNAL;
        }

        CHK_RET(ConstructPayLoadWqe(
            const_cast<void*>(remoteAddr), dstKey, const_cast<void*>(localAddr), srcKey, length, wqeType, aux,
