...
HcclResult hrtCtxDestroy(aclrtContext destroyCtx)
{
    ...
    CHK_PRT_RET(
        ret != ACL_SUCCESS && ret != ACL_ERROR_RT_CONTEXT_NULL,
        HCCL_ERROR(
            "[Get][Device]errNo[0x%016llx] aclrtDestroyContext fail,  return[%d]", HCCL_ERROR_CODE(HCCL_E_RUNTIME),
            ret),
        HCCL_E_RUNTIME);
...
HcclResult hrtGetDeviceInfo(
    u32 deviceId, HcclRtDeviceModuleType hcclModuleType, HcclRtDeviceInfoType hcclInfoType, s64& val)
{
#ifndef HCCD
