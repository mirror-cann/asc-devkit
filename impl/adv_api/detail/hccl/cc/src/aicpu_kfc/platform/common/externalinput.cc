...
HcclResult ParseRDMATrafficClass()
{
    ...
    CHK_PRT_RET(
        (ret != HCCL_SUCCESS || rdmaTrafficClass < HCCL_RDMA_TC_MIN || rdmaTrafficClass > HCCL_RDMA_TC_MAX),
        HCCL_ERROR(
...
HcclResult ParseRDMAServerLevel()
{
    ...
    CHK_PRT_RET(
        (ret != HCCL_SUCCESS || rdmaServerLevel < HCCL_RDMA_SL_MIN || rdmaServerLevel > HCCL_RDMA_SL_MAX),
        HCCL_ERROR(
...
const u32 GetExternalInputQpsPerConnection() { return g_externalInput.qpsPerConnection; }

const u32 GetExternalInputMultiQpThreshold() { return g_externalInput.multiQpThreshold; }
