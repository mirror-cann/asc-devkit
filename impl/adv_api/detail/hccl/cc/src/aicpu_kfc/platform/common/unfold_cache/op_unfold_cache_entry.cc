...
HcclResult OpUnfoldCacheEntry::UpdateRefreshAddrInfoForAlltoallv(
    const uint32_t curRank, AlltoallvMetadata& alltoallvMetadata)
{
    // 校验入参
        ...
        for (size_t sqeIdx = 0; sqeIdx < sqeCount; ++sqeIdx) {
            ...
            if (srcRefreshAddrInfo.memType == RefreshAddrInfo::USER_INPUT_MEMTYPE &&
                ...
                CHK_PRT_RET(
                    srcRank != curRank,
                    HCCL_ERROR(
                        "[OpUnfoldCacheEntry][UpdateRefreshAddrInfoForAlltoallv] srcRank[%u] != srcRank[%u]", srcRank,
                        rankSize),
                    HCCL_E_INTERNAL);
