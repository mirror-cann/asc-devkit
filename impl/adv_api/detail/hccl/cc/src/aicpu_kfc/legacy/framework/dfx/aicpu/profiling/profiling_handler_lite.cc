...
void ProfilingHandlerLite::ReportHcclOpInfo(const DfxOpInfo& opInfo) const
{
    ...
    HCCL_INFO(
        "[ProfilingHandlerLite][ReportHcclOpInfo] relay:%u, retry:%u, dataType:%s, algType:%u, count:%llu, "
...
void ProfilingHandlerLite::ReportHcclTaskDetails(const std::vector<TaskInfo>& taskInfo) const
{
    ...
    for (std::vector<Hccl::TaskInfo>::size_type i = 0; i < taskInfo.size(); i++) {
        ...
        if (batchId == HCCLINFO_REPORT_BATCH_NUM || i == taskInfo.size() - 1) {
            ReportAdditionInfo(
                MSPROF_REPORT_AICPU_MC2_BATCH_HCCL_INFO, 0, taskDetailsInfos,
...
void ProfilingHandlerLite::GetTaskDetailInfos(const TaskInfo& it, MsprofAicpuHcclTaskInfo& taskDetailsInfos) const
{
    ...
    if (it.dfxOpInfo_->isIndop_ == true) {
        taskDetailsInfos.groupName =
            GetProfHashId(it.dfxOpInfo_->groupName_.c_str(), it.dfxOpInfo_->groupName_.length());
        taskDetailsInfos.rankSize = it.dfxOpInfo_->rankSize_;
        HCCL_INFO(
            "ProfilingHandlerLite::GetTaskDetailInfos groupName_ %s, rankSize[%u]", it.dfxOpInfo_->groupName_.c_str(),
            taskDetailsInfos.rankSize);
    ...
    if (it.taskParam_.taskType == TaskParamType::TASK_SDMA || it.taskParam_.taskType == TaskParamType::TASK_RDMA ||
        ...
        it.taskParam_.taskType == TaskParamType::TASK_REDUCE_INLINE ||
        ...
        it.taskParam_.taskType == TaskParamType::TASK_NOTIFY_RECORD ||
    ...
    const MsprofAicpuHcclTaskInfo& taskDetailsInfos, const TaskInfo& taskInfo) const
{
    ...
    HCCL_INFO(
        "[ProfilingHandlerLite]ReporttHcclTaskDetails data is: itemId[%llu], cclTag[%llu], groupName[%llu], "
        ...
        taskDetailsInfos.dstAddr, taskDetailsInfos.dataSize, taskDetailsInfos.notifyID,
        DataTypeToSerialString(taskDetailsInfos.dataType).c_str(), taskDetailsInfos.linkType,
        taskDetailsInfos.timeStamp, taskDetailsInfos.durationEstimated, taskDetailsInfos.taskId,
        taskDetailsInfos.streamId, taskDetailsInfos.planeID, OpTypeToSerialString(taskDetailsInfos.opType).c_str(),
...
void ProfilingHandlerLite::ReportMainStreamTask(const FlagTaskInfo& flagTaskInfo) const
{
    ...
    HCCL_INFO(
        "[ProfilingHandlerLite][kernelTask] aicpuKernelTaskId %lu. aicpuKernelStreamId %u", aicpuKernelTaskId,
        aicpuKernelStreamId);
    ...
    // 信息上报
    ReportAdditionInfo(
        MSPROF_REPORT_AICPU_HCCL_FLAG_TASK, ProfGetCurCpuTimestamp(), &flagtask, sizeof(MsprofAicpuHcclMainStreamTask));
...
void ProfilingHandlerLite::ReportAdditionInfo(uint32_t type, uint64_t timeStamp, const void* data, int len) const
{
    ...
    if (sret != EOK) {
        THROW<InternalException>(
            "[ProfilingHandlerLite] memcpy failed, errorno[%d], len[%u], data[%u]", sret, len,
            sizeof(reporterData.data));
    }
    HCCL_INFO(
        "[ProfilingHandlerLite][ReportAdditionInfo] level :%u, type:%u, threadId:%u, dataLen:%u, timeStamp:%u",
