...
public:
    ...
    std::vector<std::tuple<QId, QId, u32>> CreateMasterSlaveQueNotifiesRequest(
        u32 queueNum, u32 pairNum = 1, QId masterId = 0) const;

    std::vector<std::tuple<QId, QId, u32>> CreateNotifiesRequestByMap(
        std::map<std::tuple<QId, QId>, u32>& notifyRequestMap) const;

    std::vector<std::tuple<QId, QId, u32>> MergeNotifiesRequest(
