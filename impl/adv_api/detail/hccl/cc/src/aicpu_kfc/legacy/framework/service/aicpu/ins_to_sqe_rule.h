...
std::vector<std::unique_ptr<HcclSqe>> Interpret(
    const Instruction& ins, const u32 streamId, ResMgrFetcher* resMgrFetcher);
std::vector<std::unique_ptr<HcclSqe>> Interpret(
    const InsLocalCopy& ins, const u32 streamId, ResMgrFetcher* resMgrFetcher);
std::vector<std::unique_ptr<HcclSqe>> Interpret(
    const InsWriteReduce& ins, const u32 streamId, ResMgrFetcher* resMgrFetcher);
std::vector<std::unique_ptr<HcclSqe>> Interpret(
    const InsLocalPostTo& ins, const u32 streamId, ResMgrFetcher* resMgrFetcher);
...
SendRecvItemTokenInfo ConvertDataAddrRange(
    DataBuffer insDataBuffer, std::unordered_map<DataBuffer, SendRecvItemTokenInfo>& tokenInfos);
SendRecvItemTokenInfo ConvertDataAddrRange(
    DataBuffer insDataBuffer, std::unordered_map<DataBuffer, SendRecvItemTokenInfo>& tokenInfos);
} // namespace Hccl
