...
void CheakDeviceIdAndDevicePort(u32 deviceId, u32& devicePort)
{
    ...
    if (devicePort > MAX_VALUE_DEVICEPORT || devicePort < MIN_VALUE_DEVICEPORT) {
        THROW<InvalidParamsException>(StringFormat(
            "device_port [%u] is out of range [%u] to [%u]", devicePort, MIN_VALUE_DEVICEPORT, MAX_VALUE_DEVICEPORT));
    }
}
...
void NewRankInfo::Deserialize(const nlohmann::json& newRankInfoJson)
{
    ...
    TRY_CATCH_THROW(InvalidParamsException, msgdeviceport,
                    devicePort = GetJsonPropertyUInt(newRankInfoJson, "device_port", false, DEFAULT_VALUE_DEVICEPORT););
...
std::string NewRankInfo::Describe() const
{
    return StringFormat(
        ...
        "tlsStatus=%d]",
        rankId, localId, replacedLocalId, rankLevelInfos.size(), devicePort, static_cast<int>(tlsStatus));
}
