...
*/
HcclResult InsTempReduceScatterNHR::CalcSliceInfo(
    ...
    std::vector<InsQuePtr>& tempInsQues)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[RS-NHR][GenExtIns] Rank[%d] queNum mismatch: need %u, got %zu", myRank_, queNum_, tempInsQues.size()),
        HcclResult::HCCL_E_INTERNAL);
...
HcclResult InsTempReduceScatterNHR::RunNHR(std::vector<InsQuePtr>& tempInsQues)
{
    ...
    for (u64 rpt = 0; rpt < rptNum; ++rpt) {
        ...
        for (u32 s = 0; s < steps.size(); ++s) {
            ...
            CHK_PRT_RET(
                SendRecvReduce(info, tempInsQues[0], 0, true, dmaMode_),
                HCCL_ERROR(
