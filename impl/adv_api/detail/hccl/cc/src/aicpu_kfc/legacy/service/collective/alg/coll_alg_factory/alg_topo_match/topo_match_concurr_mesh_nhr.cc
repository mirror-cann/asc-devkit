    ...
    std::vector<std::vector<std::vector<RankId>>>& vTopo, std::vector<std::vector<RankId>>& virtRanks)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[CollAlgFactory] [TopoMatchConcurrMeshNHR] Rank [%d], Invalid level0Topo size[%u][%u]", myRank_,
...
HcclResult TopoMatchConcurrMeshNHR::FindMyRankLocation(
    const Matrix<RankId>& podTopo, u32 myRank, u32& row, u32& col) const
{
    row = -1;
    col = -1; // 初始化为非法值
