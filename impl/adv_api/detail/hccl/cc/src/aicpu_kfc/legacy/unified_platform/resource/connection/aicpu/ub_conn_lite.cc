...
void UbConnLite::FillCommSqe(
    UdmaSqeCommon* sqe, const RmtRmaBufSliceLite& rmt, const SqeConfigLite& cfg, u32 opCode, u32 cqeEnable)
{
    ...
    HCCL_INFO(
        "UbConnLite FillCommSqe UdmaSqeCommon sqe->cqe =%u, sqe->owner = %u sqe->opcode =%u"
        "sqe->tpn = %u, sqe->rmtObjId = %u, sqe->rmtAddrLow = %u, sqe->rmtAddrHigh = %u, sqe->placeOdr = %u"
        "sqe->compOrder = %u, sqe->fence = %u",
        sqe->cqe, sqe->owner, sqe->opcode, sqe->tpn, sqe->rmtObjId, sqe->rmtAddrLow, sqe->rmtAddrHigh, sqe->placeOdr,
        sqe->compOrder, sqe->fence);
}
    ...
    DataType dataType) const
{
    ...
    for (u64 sliceIdx = 0; sliceIdx < sliceNum; sliceIdx++) {
        ...
        RmtRmaBufSliceLite rmtSlice(rmtAddr, sliceSize, 0, rmt.GetTokenId(), rmt.GetTokenValue());
    ...
    if (lastSliceSize > 0) {
        ...
        RmtRmaBufSliceLite lastRmtSlice(
            rmt.GetAddr() + sliceNum * sliceSize, lastSliceSize, 0, rmt.GetTokenId(), rmt.GetTokenValue());
    ...
    DataType dataType) const
{
    ...
    for (u32 sliceIdx = 0; sliceIdx < sliceNum; sliceIdx++) {
        ...
        RmtRmaBufSliceLite rmtSlice(
            rmt.GetAddr() + sliceIdx * sliceSize, sliceSize, 0, rmt.GetTokenId(), rmt.GetTokenValue());
...
void UbConnLite::FillOneSqeWrite(
    ...
    UdmaSqOpcode opCode, u32 cqeEnable)
{
...
void UbConnLite::ProcessOneWqeWithNotify(
    const RmaBufSliceLite& loc, const RmtRmaBufSliceLite& rmt, const SqeConfigLite& cfg, UdmaSqeWriteWithNotify* sqe,
    const RmtRmaBufSliceLite& notify, u64 notifyData, u32 opCode, const StreamLite& stream)
{
    ...
    const RmtRmaBufSliceLite& notify, const StreamLite& stream, u64 notifyData)
{
    ...
    ProcessSlicesWithNotify(
        ...
        [&](const RmaBufSliceLite& locSlice, const RmtRmaBufSliceLite& rmtSlice) {
            ...
            ProcessOneWqeWithNotify(
                locSlice, rmtSlice, cfg, &sqe, notify, notifyData, WRITE_WITH_NOTIFY_OPCODE, stream);
        });
...
void UbConnLite::FillBatchOneWqe(
    const RmaBufSliceLite& loc, const RmtRmaBufSliceLite& rmt, const SqeConfigLite& cfg, bool isLostWqe, u32 opCode,
...
void UbConnLite::BatchProcessOneSlice(
    const RmaBufSliceLite& loc, const RmtRmaBufSliceLite& rmt, const SqeConfigLite& cfg, bool isLastSlice, u32 opCode,
    const StreamLite& stream)
{
    ...
    if (remainingSize > 0 && isLastSlice) {
        ...
        RmtRmaBufSliceLite rmtTmp(
            rmt.GetAddr() + offset, remainingSize, rmt.GetRkey(), rmt.GetTokenId(), rmt.GetTokenValue());
        FillBatchOneWqe(locTmp, rmtTmp, cfg, isLastWqe, opCode, stream);
    }
}

void UbConnLite::BatchCommDataProcess(
    const vector<RmaBufSliceLite>& loc, const vector<RmtRmaBufSliceLite>& rmt, const SqeConfigLite& cfg, u32 opCode,
    const StreamLite& stream)
{
