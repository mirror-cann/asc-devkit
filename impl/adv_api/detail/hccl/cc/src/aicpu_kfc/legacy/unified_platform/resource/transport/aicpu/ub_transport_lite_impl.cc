...
HcclResult UbTransportLiteImpl::SetAddTaskInfoCallback(
    std::function<HcclResult(u32, u32, const TaskParam&, u64)> callback)
{
...
HcclResult UbTransportLiteImpl::BuildLocRmaBufferLite(
    const uintptr_t addr, const size_t size, RmaBufferLite& rmaBufferLite) const
{
    HCCL_INFO(
        "[UbTransportLiteImpl::%s] start to find addr[0x%llx], size[0x%llx] in locBufferVec, whose size is %zu. ",
    ...
    if (!isAddrInRange) {
        HCCL_WARNING(
            "[UbTransportLiteImpl::%s] addr[0x%llx], size[0x%llx] not in any range of locBufferVec. The token of the "
...
void UbTransportLiteImpl::Post(u32 index, const StreamLite& stream)
{
    ...
    connVec[0]->InlineWrite(
        reinterpret_cast<u8*>(&inlineData), UB_INLINE_WRITE_SIZE, rmtBuffSliceLite, cfg, stream, connOut);
    // 构建rts 的 sqe
...
void UbTransportLiteImpl::ProfilingProcess(
    const RmaBufSliceLite& loc, const RmtRmaBufSliceLite& rmt, const StreamLite& stream, DmaOp dmaOp, u32 taskId)
{
    if (callback_ == nullptr && newCallback_ == nullptr) {
        HCCL_WARNING("[UbTransportLiteImpl] callback_ is nullptr.");
        return;
...
void UbTransportLiteImpl::ReduceProfilingProcess(
    const RmaBufSliceLite& loc, const RmtRmaBufSliceLite& rmt, const ReduceIn& reduceIn, const StreamLite& stream,
    u32 taskId)
{
    if (callback_ == nullptr && newCallback_ == nullptr) {
        HCCL_WARNING("[UbTransportLiteImpl] callback_ is nullptr.");
    ...
    const std::vector<BaseTransportLiteImpl::TransferOp>& transferOp, const StreamLite& stream)
{
    ...
    for (u32 i = 0; i < insNum; i++) {
        ...
        if (transferOp[i].transType == TransferType::WRITE) {
            if (transferOp[i].reduceIn.reduceOp == ReduceOp::INVALID) {
                connVec[0]->Write(
                    localBuffer, remoteBuffer, cfg, stream, connOut); // 当前只有一个connection，对应一个jetty
            } else {
                connVec[0]->WriteReduce(
    ...
    if (transferOp[insNum - 1].reduceIn.reduceOp == ReduceOp::INVALID) {
        ...
        ProfilingProcess(
            GetRmaBufSlicelite(loc[insNum - 1]), GetRmtRmaBufSliceLite(rmt[insNum - 1]), stream, dmaOp, taskId);
    } else {
        ReduceProfilingProcess(
            GetRmaBufSlicelite(loc[insNum - 1]), GetRmtRmaBufSliceLite(rmt[insNum - 1]),
            transferOp[insNum - 1].reduceIn, stream, taskId);
    }
}
    ...
    const RmaBufferLite& loc, const Buffer& rmt, const WithNotifyIn& withNotify, const StreamLite& stream)
{
    ...
    connVec[0]->WriteWithNotify(
        locRmaBufSlicelite, rmtRmaBufSlicelite, cfg, connOut, rmtNotifySliceLite, stream, notifyData);
    BuildUbDbSendTask(stream, connVec[0]->GetUbJettyLiteId(), connOut.pi);
    ...
    const StreamLite& stream)
{
    ...
    auto rmtRmaBufSlicelite = GetRmtRmaBufSliceLite(rmt);
    auto rmtNotifySliceLite = GetRmtNotifySliceLite(withNotify.index_);
    connVec[0]->WriteReduceWithNotify(
