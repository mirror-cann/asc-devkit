...
HcclResult IRankGraph::GetInstSizeListByNetLayer(uint32_t netLayer, uint32_t** instSizeList, uint32_t* listSize)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "[IRankGraph::GetInstSizeListByNetLayer] Failed to get instSzie[%u] at netLayer[%u]", listSize, netLayer);
        return ret;
    }
...
HcclResult IRankGraph::GetLinks(
...
HcclResult IRankGraph::GetRanksByTopoInst(
    const uint32_t netLayer, const uint32_t topoInstId, uint32_t** ranks, uint32_t* rankNum)
{
    CHK_PTR_NULL(rankGraphPtr_);
    RankGraph* rankGraph = static_cast<RankGraph*>(rankGraphPtr_);
    u32 rankId = rankGraph->GetMyRank();
...
HcclResult IRankGraph::GetEndpointNum(uint32_t netLayer, uint32_t topoInstId, uint32_t* num)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "[IRankGraph::GetEndpointNum] Faild to get endpoint num at netLayer [%u] with topoInstId", netLayer,
            topoInstId);
        return ret;
    }
    return HCCL_SUCCESS;
...
HcclResult IRankGraph::GetEndpointDesc(
    uint32_t netLayer, uint32_t topoInstId, uint32_t* descNum, EndpointDesc* endpointDesc)
{
    CHK_PTR_NULL(rankGraphPtr_);
    RankGraph* rankGraph = static_cast<RankGraph*>(rankGraphPtr_);
    u32 rankId = rankGraph->GetMyRank();
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "[IRankGraph::GetEndpointDesc] Failed to get endpoint desc at netLayer [%u] with descNum [%u]", netLayer,
            descNum);
        return ret;
    }
    return HCCL_SUCCESS;
