...
static inline u32 GetA5SqeType(Rt91095StarsSqeType sqeType) { return static_cast<u32>(static_cast<u8>(sqeType)); }

void LogAicpuOrderDfxSqe(
    const char* stage, u32 streamId, u32 taskId, u32 sqeType, u32 sqId, u32 sqHead, u32 sqTail, u32 pendingSqeCnt)
{
    if (LIKELY(!HcclCheckLogLevel(HCCL_LOG_INFO))) {
        return;
    }
    HCCL_INFO(
        "[AICPU_ORDER_DFX][SQE] stage[%s], streamId[%u], taskId[%u], sqeType[%u], sqId[%u], "
...
void RtsqA5::MakeSureAvailableSpace()
{
    ...
    HCCL_INFO(
        "RtsqA5::%s timeout: %u s, cur head: %u, tail: %u, sqId: %u", __func__, timeoutValue, sqHead_, sqTail_, sqId_);
    ...
    while (availableSpace <= pendingSqeCnt) {
        if (UNLIKELY(std::chrono::steady_clock::now() - lastPrintTime >= printInterval)) {
            HCCL_INFO(
                "RtsqA5::%s while loop availableSpace %u <= pendingSqeCnt %u", __func__, availableSpace, pendingSqeCnt);
            lastPrintTime = std::chrono::steady_clock::now();
        }
        if (UNLIKELY(
                (std::chrono::steady_clock::now() - startTime) >= timeout)) { // timeout内还是不能向RTSQ中写入值，报错
...
void RtsqA5::CopyLocBufToSq()
{
    ...
    if (sqTail_ >= sqHead_) {
        ...
        if (pendingSqeCnt <= depthLeft) { // 没有回绕
            ...
            // 拷贝剩余sqe
            ret = memcpy_sp(
                reinterpret_cast<u8*>(sqBaseAddr_), sqHead_ * rtsqSqeSize, locBuf + depthLeft * rtsqSqeSize,
...
void RtsqA5::LaunchTask()
{
    ...
    LogAicpuOrderDfxSqe(
        "LaunchTaskStart", streamId_, taskId_, RTSQ_A5_INVALID_SQE_TYPE, sqId_, sqHead_, sqTail_, pendingSqeCnt);

    if (pendingSqeCnt == 0) { // 没有SQE ，直接返回
        HCCL_INFO("RtsqA5::%s: pendingSqeCnt is %u, return", __func__, pendingSqeCnt);
        LogAicpuOrderDfxSqe(
            "LaunchTaskEmpty", streamId_, taskId_, RTSQ_A5_INVALID_SQE_TYPE, sqId_, sqHead_, sqTail_, pendingSqeCnt);
        return;
    }
    ...
    LogAicpuOrderDfxSqe(
        "LaunchTaskEnd", streamId_, taskId_, RTSQ_A5_INVALID_SQE_TYPE, sqId_, sqHead_, sqTail_, pendingSqeCnt);
}
...
void RtsqA5::TryLaunchTask()
{
    ...
    LogAicpuOrderDfxSqe(
        "TryLaunchTaskStart", streamId_, taskId_, RTSQ_A5_INVALID_SQE_TYPE, sqId_, sqHead_, sqTail_, pendingSqeCnt);

    if (pendingSqeCnt == 0) {
        HCCL_DEBUG("RtsqA5::%s: pendingSqeCnt is %u, return", __func__, pendingSqeCnt);
        LogAicpuOrderDfxSqe(
            "TryLaunchTaskEmpty", streamId_, taskId_, RTSQ_A5_INVALID_SQE_TYPE, sqId_, sqHead_, sqTail_, pendingSqeCnt);
        return;
    }
    ...
    if (availableSpace <= pendingSqeCnt) {
        HCCL_DEBUG(
            "RtsqA5::%s: no enough space, availableSpace[%u] < pendingSqeCnt[%u], return", __func__, availableSpace,
            pendingSqeCnt);
        return;
    }
    ...
    HCCL_INFO("RtsqA5::%s: END, pendingSqeCnt[%u], sqHead_[%u] sqTail_[%u]", __func__, pendingSqeCnt, sqHead_, sqTail_);
    LogAicpuOrderDfxSqe(
        "TryLaunchTaskEnd", streamId_, taskId_, RTSQ_A5_INVALID_SQE_TYPE, sqId_, sqHead_, sqTail_, pendingSqeCnt);
}

u8* RtsqA5::GetCurrSqeBuffer() { return locBuf + pendingSqeCnt * rtsqSqeSize; }
...
void RtsqA5::NotifyWait(u32 notifyId, u32 timeout)
{
    ...
    HCCL_INFO(
        "RtsqA5::NotifyWait: streamId %u, taskId %u, notifyId %u, timeout %u", streamId_, taskId_, notifyId, timeout);
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[NotifyWait], streamId[%u], taskId[%u], sqeType[%u], "
            "sqId[%u], sqHead[%u], sqTail[%u], pendingSqeCnt[%u], notifyId[%u], timeout[%u].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_NOTIFY_WAIT), sqId_, sqHead_,
            sqTail_, pendingSqeCnt, notifyId, timeout);
    }
    RefreshInfo();
}
...
void RtsqA5::NotifyRecordLoc(u32 notifyId)
{
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[NotifyRecordLoc], streamId[%u], taskId[%u], sqeType[%u], "
            "sqId[%u], sqHead[%u], sqTail[%u], pendingSqeCnt[%u], notifyId[%u].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_NOTIFY_RECORD), sqId_, sqHead_,
            sqTail_, pendingSqeCnt, notifyId);
    }
    RefreshInfo();
}
...
void RtsqA5::SdmaCopy(u64 srcAddr, u64 dstAddr, u32 size, u32 partId)
{
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[SdmaCopy], streamId[%u], taskId[%u], sqeType[%u], sqId[%u], "
            "sqHead[%u], sqTail[%u], pendingSqeCnt[%u], srcAddr[0x%llx], dstAddr[0x%llx], size[%u], "
            "partId[%u], dataType[none], reduceOp[none].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_SDMA), sqId_, sqHead_, sqTail_,
            pendingSqeCnt, srcAddr, dstAddr, size, partId);
    }
    RefreshInfo();
}
...
void RtsqA5::SdmaReduce(u64 srcAddr, u64 dstAddr, u32 size, u32 partId, const ReduceIn& reduceIn)
{
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[SdmaReduce], streamId[%u], taskId[%u], sqeType[%u], sqId[%u], "
            "sqHead[%u], sqTail[%u], pendingSqeCnt[%u], srcAddr[0x%llx], dstAddr[0x%llx], size[%u], "
            "partId[%u], dataType[%s], reduceOp[%s].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_SDMA), sqId_, sqHead_, sqTail_,
            pendingSqeCnt, srcAddr, dstAddr, size, partId, reduceIn.dataType.Describe().c_str(),
...
void RtsqA5::UbDbSend(const UbJettyLiteId& jettyLiteId, u16 piValue)
{
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[UbDbSend], streamId[%u], taskId[%u], sqeType[%u], sqId[%u], "
            "sqHead[%u], sqTail[%u], pendingSqeCnt[%u], piValue[%u].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_UBDMA), sqId_, sqHead_, sqTail_,
            pendingSqeCnt, piValue);
    }
    RefreshInfo();
}
...
void RtsqA5::CCoreNotifyWait(u64 waitAddr, u64 curTurnCntAddr, bool last)
{
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[CCoreNotifyWait], streamId[%u], taskId[%u], sqeType[%u], "
            "sqId[%u], sqHead[%u], sqTail[%u], pendingSqeCnt[%u], waitAddr[0x%llx], valueAddr[0x%llx], "
            "last[%d].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_COND), sqId_, sqHead_, sqTail_,
            pendingSqeCnt, waitAddr, curTurnCntAddr, last);
    }
    RefreshInfo();
}
...
void RtsqA5::CCoreNotifyRecord(u64 recordAddr, u64 curTurnCntAddr)
{
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        HCCL_INFO(
            "[AICPU_ORDER_DFX][SQE] stage[CCoreNotifyRecord], streamId[%u], taskId[%u], sqeType[%u], "
            "sqId[%u], sqHead[%u], sqTail[%u], pendingSqeCnt[%u], recordAddr[0x%llx], valueAddr[0x%llx].",
            streamId_, taskId_, GetA5SqeType(Rt91095StarsSqeType::RT_91095_SQE_TYPE_COND), sqId_, sqHead_, sqTail_,
            pendingSqeCnt, recordAddr, curTurnCntAddr);
    }
    RefreshInfo();
}
...
void RtsqA5::P2PWriteValue(u64 remoteAddr, u32 writeValue)
{
    ...
    HCCL_INFO(
        "RtsqA5::P2PWriteValue: streamId %u, taskId %u, remoteAddr %llu, writeValue %llu", streamId_, taskId_,
        remoteAddr, writeValue);
    RefreshInfo();
}
} // namespace Hccl
