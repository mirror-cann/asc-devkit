...
uint64_t InsTempScatterNHR::GetExpandedMode() const { return DeviceMode::AICPU; }
...
HcclResult InsTempScatterNHR::PreCopy(TemplateDataParams& templateDataParams, std::vector<InsQuePtr>& tempInsQues)
{
    ...
    for (u32 r = 0; r < templateDataParams.repeatNum; r++) {
        for (u32 algRank = 0; algRank < tempRankSize_; algRank++) {
            DataSlice srcSlice(
                BufferType::INPUT,
                r * templateDataParams.inputRepeatStride + templateDataParams.inputSliceStride * algRank +
                    buffInfo_.inBuffBaseOff,
                templateDataParams.sliceSize);
            DataSlice dstSlice(
                BufferType::SCRATCH,
                r * tempRankSize_ * templateDataParams.sliceSize + buffInfo_.scratchBuffBaseOff +
                    algRank * templateDataParams.sliceSize,
                templateDataParams.sliceSize);
...
HcclResult InsTempScatterNHR::PostCopy(TemplateDataParams& templateDataParams, std::vector<InsQuePtr>& tempInsQues)
{
    ...
    for (u32 r = 0; r < templateDataParams.repeatNum; r++) {
        ...
        u64 srcOffset = r * tempRankSize_ * templateDataParams.sliceSize + buffInfo_.scratchBuffBaseOff +
                        myAlgRank * templateDataParams.sliceSize;
    ...
    u32 repeat) const
{
    ...
    for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
        ...
        u64 srcDstOffset = repeat * tempRankSize_ * templateDataParams.sliceSize + buffInfo_.scratchBuffBaseOff +
                           txId * templateDataParams.sliceSize;
    ...
    u32 repeat) const
{
    ...
    for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
        ...
        u64 srcDstOffset = repeat * tempRankSize_ * templateDataParams.sliceSize + buffInfo_.scratchBuffBaseOff +
                           rxId * templateDataParams.sliceSize;
        DataSlice srcDstSlice(BufferType::SCRATCH, srcDstOffset, templateDataParams.sliceSize);
        srcDstSlices.push_back(srcDstSlice);
    }
