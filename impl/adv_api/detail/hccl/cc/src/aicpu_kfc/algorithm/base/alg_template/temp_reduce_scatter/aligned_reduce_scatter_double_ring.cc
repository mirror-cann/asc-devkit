...
HcclResult AlignedReduceScatterDoubleRing::CheckParameters(
    ...
    u32 initSlice1Idx, DeviceMem& dstInit, DeviceMem& srcInit, DeviceMem& dstSubInit, DeviceMem& srcSubInit)
{
    ...
    if (rankSize == TWO_RANK_SIZE && opInfo_->outputAddr != nullptr) {
        ...
        dstInit =
            DeviceMem::create(static_cast<u8*>(opInfo_->outputAddr) + lastStepOffsets_[ringIndex], dstInitSlice1.size);
    } else {
        HCCL_DEBUG(
            "Memcpy operation: step[-1] stream[main] src rank[%u] starts to copy(rcv) offset[%llu], size[%llu] on "
...
HcclResult AlignedReduceScatterDoubleRing::PrepareRunMainStream(
