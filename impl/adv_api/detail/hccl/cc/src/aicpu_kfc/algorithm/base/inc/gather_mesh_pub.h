...
protected:
private:
    HcclResult RunRecvGather(
        const u32 srcRank, const Slice& slice, const std::vector<std::shared_ptr<Transport>>& links);
    HcclResult RunSendGather(
        const u32 dstRank, const Slice& slice, const std::vector<std::shared_ptr<Transport>>& links);
