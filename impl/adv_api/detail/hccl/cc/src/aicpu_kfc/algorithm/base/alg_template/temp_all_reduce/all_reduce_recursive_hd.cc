...
// 算法的主入口
HcclResult AllReduceRecursiveHalvingDoubling::RunAsync(
...
HcclResult AllReduceRecursiveHalvingDoubling::PrepareRunAsync(
    const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
...
HcclResult AllReduceRecursiveHalvingDoubling::ReduceScatterInBlock(
...
HcclResult AllReduceRecursiveHalvingDoubling::AllGatherInBlock(u32 rank, u32 rankSize, const std::vector<LINK>& links)
{
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg =
        AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_ALL_GATHER_HALVING_DOUBLING, dispatcher_);
    CHK_SMART_PTR_NULL(tempAlg);
    ...
    const u32 rank, const u32 rankSize, const std::vector<LINK>& links, AdjInfo& nslbAdjInfo)
{
    ...
    if (rank < nslbPart1Size && (rank % NSLBDP_ALL_REDUCE_MOLD2) == 0) {
        ...
        rankInBlock =
            rank - nslbPart1Size / NSLBDP_ALL_REDUCE_MOLD2; // 通过rank减去part1除2的大小即不处于第一部分的block内rank号
    }
    std::vector<LINK> subLinks;
