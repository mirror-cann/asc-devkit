...
HcclResult ReduceScatterRingConcurrentDirect::RunAsync(
...
HcclResult ReduceScatterRingConcurrentDirect::CheckParameters(
    const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
    ...
    const u32 rankSize)
{
    ...
    for (u32 sliceIdx = 0; sliceIdx < subSliceVector.size(); sliceIdx++) {
        ...
        if (step == rankSize - DMA_REDUCE_TWO_OFFSET) {
            ...
            dst = DeviceMem::create(
                static_cast<u8*>(opInfo_->outputAddr) + lastStepOffset_, subSliceVector[sliceIdx].size);
