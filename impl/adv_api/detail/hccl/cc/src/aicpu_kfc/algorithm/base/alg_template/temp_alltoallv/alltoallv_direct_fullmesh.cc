...
HcclResult AlltoAllVDirectFullMesh::Prepare(PrepareData& param)
{
    ...
    u32 blockGroup =
        (isBigCount_ || opType_ == HcclCMDType::HCCL_CMD_ALLTOALLV || opType_ == HcclCMDType::HCCL_CMD_ALLTOALLVC) ? 2 :
                                                                                                                     1;
...
HcclResult AlltoAllVDirectFullMesh::CopyDataForSend(
    u32 dstRank, std::vector<SendDataBlock>& sendInfo, u32 curStep, Stream stream)
{
...
HcclResult AlltoAllVDirectFullMesh::ProcessSingleGroupRdmaData(
    std::vector<u32>& dstRanks, std::vector<u32>& srcRanks, u32 round)
{
...
HcclResult AlltoAllVDirectFullMesh::RunSDMATasks(u32 roundIdx, u32 step, u32 groupRankSize, u32 leftRankSize)
{
    if (isBigCount_) {
        ...
        if (roundIdx < commRounds_ - 1) {
            u32 nextgroupRankSize =
                (leftRankSize - groupRankSize > sdmaConcurrentNum_) ? sdmaConcurrentNum_ : leftRankSize - groupRankSize;
