...
HcclResult BcastRecursiveHalvingDoubling::ReceiveData(
    const u32 destRank, const std::vector<std::shared_ptr<Transport> >& links)
{
...
HcclResult BcastRecursiveHalvingDoubling::SendData(
    const u32 destRank, const std::vector<std::shared_ptr<Transport> >& links)
{
    if (destRank < links.size()) {
        if (links[destRank] == nullptr) {
...
HcclResult BcastRecursiveHalvingDoubling::BroadcastInBlock(
