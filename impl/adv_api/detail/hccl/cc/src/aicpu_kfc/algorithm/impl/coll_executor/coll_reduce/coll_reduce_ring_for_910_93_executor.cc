...
HcclResult CollReduceRingFor91093Executor::CalcTransportMemType(
...
HcclResult CollReduceRingFor91093Executor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    if (sliceNum > 1 &&
        (level0TransportInfo.userRank2subCommRank.find(param.root) != level0TransportInfo.userRank2subCommRank.end())) {
...
HcclResult CollReduceRingFor91093Executor::SelectTempAlg(
