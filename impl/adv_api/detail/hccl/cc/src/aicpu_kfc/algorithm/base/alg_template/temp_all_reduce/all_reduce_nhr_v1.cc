...
HcclResult AllReduceNHRV1::RunAsync(
...
HcclResult AllReduceNHRV1::RunAsyncStaged(
...
HcclResult AllReduceNHRV1::RunReduceScatterOnHorizontal(
    const u32 rank, const std::vector<LINK>& links, const RingInfo& info)
{
...
HcclResult AllReduceNHRV1::RunAllGatherOnHorizontal(
    const u32 rank, const std::vector<LINK>& links, const RingInfo& info)
{
    u32 ringRank = info.GetHIndex(rank); // 查找自己位于第几列，也即处于Ring中的第几个rank
...
HcclResult AllReduceNHRV1::RunAllGatherBrokenRing(
    const u32 rank, const std::vector<LINK>& links, const std::vector<Slice>& slices)
{
    std::unique_ptr<AlgTemplateBase> tempAlg =
