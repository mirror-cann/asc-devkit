    ...
    std::vector<std::vector<Slice>>& level0DataSegsSlice)
{
    ...
    multiStreamSlice =
        ReduceScatterRingSlicePrepare(ringNum, sliceNum, useInlineReduce, execMem.outputMem, dataSegsSlice, param.tag);
...
HcomCollOpInfo CollReduceScatterRingFor91093Executor::GetHcomCollOpInfo(
    const OpParam& param, const ExecMem& execMem) const
{
    ...
    HcomCollOpInfo opInfo = {"",       execMem.inputPtr, execMem.outputPtr, count,
                             dataType, param.root,       param.reduceType,  strideCount};
    HCCL_DEBUG(
...
HcclResult CollReduceScatterRingFor91093Executor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    bool disableDMAReduce =
        algOpContext_.opRetryHandler.retryEnable && (algOpContext_.opRetryHandler.inPlaceSupportRetryStatus ==
