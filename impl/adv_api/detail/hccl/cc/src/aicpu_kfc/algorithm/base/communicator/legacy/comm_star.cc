...
CommStar::CommStar(
    ...
    const std::string& tag, const NICDeployment nicDeployInner, const u32 subUserRankRoot, bool isHaveCpuRank)
    : CommBase(
          ...
          transportResourceInfoSize, tag, nicDeployInner, false, false, false, subUserRankRoot, isHaveCpuRank)
{
...
void CommStar::SetTransportParam(TransportPara& para, MachinePara& machinePara)
{
    ...
    if (paraVector_[rank_].devicePhyId == HOST_DEVICE_ID) {
        ...
        if (paraVector_[rank_].deviceType == DevType::DEV_TYPE_910B) {
            ...
            para.qpMode =
                (GetWorkflowMode() == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE ? OPBASE_QP_MODE : OFFLINE_QP_MODE);
        }
