...
HcclResult CommAHCBaseInfo::GetNBNslbDstRanks(
    const u32 rank, const std::vector<u32> commGroups, std::vector<u32>& dstRanks)
{
    CHK_PRT_RET(
...
HcclResult CommAHCBaseInfo::GetNHRNslbDstRanks(
    const u32 rank, const std::vector<u32> commGroups, std::vector<u32>& dstRanks)
{
    CHK_PRT_RET(
        rank >= commGroups.size(),
