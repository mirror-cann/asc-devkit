...
CommBase::CommBase(
    ...
    const NICDeployment nicDeployInner, bool isAlltoAllCommMesh, const bool useOneDoorbell, const bool isAicpuModeEn,
    const u32 rankRoot, const bool isHaveCpuRank, const bool useSuperPodMode, DeviceMem expMem)
    : linkDummy_(nullptr),
...
HcclResult CommBase::CreateIntraThread(
    const u32 role, u32 dstRank, const std::vector<std::shared_ptr<HcclSocket>>& sockets)
{
...
HcclResult CommBase::CreateInterThread(
    const u32 role, u32 dstRank, const std::vector<std::shared_ptr<HcclSocket>>& sockets)
{
    if (sockets.empty()) {
    ...
    const std::vector<std::shared_ptr<HcclSocket>>& sockets, u32& status)
{
    ...
    para.timeout = kdefaultTimeout;
    para.virtualFlag = false;
    transportInfo_[dstRank].reset(new (std::nothrow) Transport(
        TransportType::TRANS_TYPE_HETEROG_P2P, para, dispatcher_, notifyPool_, machinePara));
    CHK_PRT_RET(!transportInfo_[dstRank], HCCL_ERROR("[Init][Transport]In create link, new link failed"), HCCL_E_PTR);
    ...
    std::map<u32, HcclRankLinkInfo>& dstClientMap)
{
    ...
    for (auto rankIter = rankRole.begin(); rankIter != rankRole.end(); rankIter++) {
        ...
        s32 dstDeviceId =
            useSuperPodMode_ ? static_cast<s32>(paraVector_[dstRank].superDeviceId) : paraVector_[dstRank].devicePhyId;
