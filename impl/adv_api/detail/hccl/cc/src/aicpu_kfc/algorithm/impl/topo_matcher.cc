...
HcclResult TopoMatcher::GetIsUsedRdma(const CommParaInfo& commParaInfo, bool& isUsedRdma)
{
    ...
    std::vector<std::vector<u32>>& commPlaneVec =
        (commParaInfo.commType == CommType::COMM_TAG_P2P) ? commP2PPlaneVec : CommPlaneVector_[commParaInfo.commPlane];
...
HcclResult TopoMatcher::SetIsUsedRdma(
...
HcclResult TopoMatcher::GetSubRootForScatter(const u32 root, u32& subRoot)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ringIndex++) {
        if (isBridgeVector_[ringIndex]) {
            rank =
                GetSubCollectiveRank(CommPlaneVector_[COMM_LEVEL1_INDEX][ringIndex]); // 确定userRank在level1中的rank号
        }
...
HcclResult TopoMatcher::GetGlobalSubGroups(
    const CommPlane level, std::vector<std::vector<std::vector<u32>>>& globalSubGroups)
{
...
HcclResult TopoMatcher::SetGlobalSubGroups(
    const CommPlane level, std::vector<std::vector<std::vector<u32>>>& globalSubGroups)
{
