...
{
    u32 totalStreamNum =
        1 + (topoType_ == TopoType::TOPO_TYPE_NP_DOUBLE_RING ? (LEVEL0_PLANE_NUM_IN_NPRING_DOUBLE + 1) :
...
HcclResult CollAllGatherRingZerocopyPipelineExecutor::KernelRunWithLoop(
    const OpParam& param, ExecMem& execMem, bool isLastLoop)
{
...
/* 数据交换 */
HcclResult CollAllGatherRingZerocopyPipelineExecutor::KernelRunInterServerPreProcess(
...
HcclResult CollAllGatherRingZerocopyPipelineExecutor::KernelRunInterServerPostProcess(
    const OpParam& param, ExecMem& execMem)
{
    (void)param;
