...
public:
    ...
    // AICPU侧创建Transport
    static std::shared_ptr<TransportMem> Create(
        TpType tpType, const HcclQpInfoV2& qpInfo, const HcclDispatcher& dispatcher, AttrInfo& attrInfo);
    ...
    virtual HcclResult GetTransInfo(
        HcclQpInfoV2& qpInfo, u32* lkey, u32* rkey, HcclBuf* localMem, HcclBuf* remoteMem, u32 num) = 0;
    virtual HcclResult WaitOpFence(const rtStream_t& stream) = 0;

    // AICPU侧批量下发读、写操作，下发wr后敲Doorbell
    virtual HcclResult BatchWrite(
        const std::vector<MemDetails>& remoteMems, const std::vector<MemDetails>& localMems, Stream& stream) = 0;
    virtual HcclResult BatchRead(
        const std::vector<MemDetails>& localMems, const std::vector<MemDetails>& remoteMems, Stream& stream) = 0;
    virtual HcclResult AddOpFence(
        const MemDetails& localFenceMem, const MemDetails& remoteFenceMem, Stream& stream) = 0;
