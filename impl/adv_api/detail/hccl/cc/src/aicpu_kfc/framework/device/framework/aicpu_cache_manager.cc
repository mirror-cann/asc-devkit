    ...
    const HcclWorkflowMode workflowMode, bool& needCache)
{
    ...
    if (param.isZeroCopy && opType == HcclCMDType::HCCL_CMD_REDUCE_SCATTER) {
        HCCL_INFO(
            "[AicpuCacheManager][%s] zcopy[%d] + opType[%d] is not supported for unfolding cache", __func__,
            param.isZeroCopy, opType);
        return HCCL_SUCCESS;
    }
    ...
    const DeviceMem& tinySendRecvMem)
{
    ...
    if (param.isZeroCopy) {
        ...
        param.aicpuCacheEnable > FORCE_OP_BASE_DELTA ||
        param.isCapture) { // 存在强制单算子模式转换 (即图模式建链+单算子模式展开)
