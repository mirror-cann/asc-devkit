...
HcclResult MC2TraceUtils::GetTraceFunc(const std::string& traceName)
{
    ...
    g_traceSetGlobalAttr =
        reinterpret_cast<TraceSetGlobalAttrFunc>(mmDlsym(g_soHandle, std::string(traceName + "SetGlobalAttr").c_str()));
    CHK_PRT_RET(
    ...
    CHK_PRT_RET(
        ...
        HCCL_E_SYSCALL);
    g_traceStructSetAttr =
        reinterpret_cast<TraceStructSetAttrFunc>(mmDlsym(g_soHandle, std::string(traceName + "StructSetAttr").c_str()));
    CHK_PRT_RET(
        ...
        for (uint32_t i = 0U; i <= submitNum / MAX_SQE_BATCH_SIZE; i++) {
            ...
            for (uint32_t j = 0U; j < MAX_SQE_BATCH_SIZE; j++) {
                ...
                if (AicpuSqeContext::QuerySqeInfoByTaskId(
                        streamId, buff.tailSqeTaskId - submitNum + idx, &sqeBatchInfo.sqeInfos[j]) != HCCL_SUCCESS) {
