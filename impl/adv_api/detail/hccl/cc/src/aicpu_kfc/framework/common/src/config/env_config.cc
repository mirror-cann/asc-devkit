...
HcclResult CheckSocketPortRangeValid(const std::string& envName, const std::vector<HcclSocketPortRange>& portRanges)
{
    ...
    for (size_t i = 0; i < rangeVec.size(); ++i) {
        ...
        CHK_PRT_RET(
            rangeVec[i].min <= HCCL_SOCKET_PORT_RANGE_AUTO && rangeVec[i].max >= HCCL_SOCKET_PORT_RANGE_AUTO,
            HCCL_ERROR(
...
HcclResult ParseSingleDFSConfigItem(
    const std::string& dfsConfigEnv, const std::string& configName, std::string& configResult)
{
...
HcclResult ParseMonitor(std::string& taskMonitorInterval, s32& monitorTime)
{
    ...
    if (monitorTime == 0) {
        ...
        HCCL_ERROR(
            "[ParseDFSConfig] HCCL_DFS_CONFIG-task_monitor_interval[%d] is invalid, except: [0, %d]", monitorTime,
            maxTimeInMs);
        return HCCL_E_PARA;
    }
    return HCCL_SUCCESS;
...
const bool& GetExternalInconsistentCheckSwitch() { return g_envConfig.inconsistentCheckSwitch; }
