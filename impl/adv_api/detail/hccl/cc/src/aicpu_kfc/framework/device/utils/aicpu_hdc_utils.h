...
public:
    ...
    static HcclResult GetSuspendingStatus(
        std::shared_ptr<hccl::HDCommunicate> h2dTransfer, HcclComSuspendingFlag& flag);
    static HcclResult GetBackGroundCommand(std::shared_ptr<hccl::HDCommunicate> h2dTransfer, BackgroundCommand& bgCmd);
    static HcclResult ResponseBackGroundStatus(
        std::shared_ptr<hccl::HDCommunicate> d2hTransfer, KfcExecStatus& kfcStatus);
