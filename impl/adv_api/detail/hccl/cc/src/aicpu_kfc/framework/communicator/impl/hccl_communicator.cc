...
HcclResult HcclCommunicator::InitTransportManager()
{
    ...
    transportManager_.reset(static_cast<TransportManager*>(new (std::nothrow) TransportManager(
        ...
        deviceLogicId_, nicDeployment_, isHaveCpuRank_, static_cast<const void*>(&transportResInfo_),
        sizeof(transportResInfo_), isUseRankPort_, isUsedRdmaLevel0_, nicRanksPorts, vnicRanksPorts, useSuperPodMode_,
...
HcclResult HcclCommunicator::InitHcclAlg()
{
    ...
    CHK_RET(implAlg_->Init(
        static_cast<const void*>(&transportResInfo_), sizeof(transportResInfo_), workSpaceRes_, notifyPool_,
        netDevCtxMap_, queueNotifyManager_, algoAttr, topoAttr, false));
    return HCCL_SUCCESS;
}
...
HcclResult HcclCommunicator::InitRankInfoSubGroup(
...
HcclResult HcclCommunicator::SetInfoToDevice(
    const OpParam& opParam, const std::unique_ptr<PreProcessMetaInfo>& preMetaInfo, const HcclWorkflowMode& mode,
    Stream& stream)
{
...
HcclResult HcclCommunicator::GetInfoFromDevice(
    const OpParam& opParam, const std::unique_ptr<PreProcessMetaInfo>& preMetaInfo, const HcclWorkflowMode& mode,
    Stream& stream, HostMem& hostCollectBuffer)
{
    CHK_RET(hrtMemSyncCopy(
...
HcclResult HcclCommunicator::updateList(u64 size, void* buffer) const { return HCCL_SUCCESS; }
    ...
    const OpParam& opParam, const HcclCMDType& opType, const u32& rankSize, const std::string& algName)
{
    ...
    if (opType == HcclCMDType::HCCL_CMD_BATCH_SEND_RECV) {
        dynamicDataSize = sizeof(struct OpTilingBatchSendRecvDataDes) +
                          opParam.BatchSendRecvDataDes.itemNum * sizeof(HcclSendRecvItem) + userRankSize_ * sizeof(u8);
...
HcclResult HcclCommunicator::SetSignalTransport(
    SingleSubCommTransport& singleSubCommTransport, u32 linkIdx, bool statusStop)
{
...
HcclResult HcclCommunicator::Resume()
{
    ...
    HcclUs endut = TIME_NOW();
    HCCL_RUN_INFO("HcclCommunicator::Resume, Resume take time:[%lld]us", DURATION_US(endut - startut).count());
...
HcclResult HcclCommunicator::GetTransportCqeErrors(
    const HcclNetDevCtx netDevCtx, std::vector<ErrCqeInfo>& infos, u32& num)
{
    ...
    for (auto& info : infolist) {
        ...
        if (iter != linkResMap_.end()) {
            ...
            HCCL_RUN_WARNING(
                "[GetTransportCqeErrors]get err failed, transport is not find, localIp[%s], remoteIp[%s]",
