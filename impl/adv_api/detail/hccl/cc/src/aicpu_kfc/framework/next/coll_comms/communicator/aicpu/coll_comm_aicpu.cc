...
HcclResult CollCommAicpu::ProcessUrmaRes(HcclChannelUrmaRes* commParam, bool isInit)
{
    ...
    for (u32 index = 0; index < commParam->listNum; index++) {
        ...
        // 打印
        HCCL_INFO(
            "[CollCommAicpu][%s] index[%u], currentSrcAddr[%p], singleUniqueIdSize[%u], channelHandle[0x%llx]",
...
HcclResult CollCommAicpu::NotifyFree(NotifyMgrAicpuParam* param)
{
    ...
    for (size_t i = 0; i < notifyNum; ++i) {
        ...
        if (it != notifys_.end()) {
            HCCL_INFO(
                "[CollCommAicpu][%s] comm identifier[%s], free notifys[%u] success", __func__, hcomId.c_str(),
                notifyArray[i]);
            notifys_.erase(it);
        } else {
...
HcclResult CollCommAicpu::NotifyAlloc(NotifyMgrAicpuParam* param)
{
    ...
    HCCL_INFO(
        "[CollCommAicpu][%s] comm identifier[%s], alloc notifys num[%u] begin, before notifySize[%u]", __func__,
        hcomId.c_str(), notifyNum, notifySize);
    HcclResult ret = NotifyManager::ParseBinNotifys(notifysStr, notifys_);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "[CollCommAicpu][%s] comm identifier[%s], alloc notifys num[%u] failed %u", __func__, hcomId.c_str(),
            notifyNum, ret);
        return ret;
    }
    HCCL_INFO(
        "[CollCommAicpu][%s] comm identifier[%s], alloc notifys num[%u] end, after notifySize[%u]", __func__,
...
void CollCommAicpu::InitBackGroundThread()
{
    ...
    static auto daemonServiceRun = [](void* info) { Hccl::AicpuDaemonService::GetInstance().ServiceRun(info); };
    static auto daemonServiceStop = [](void* info) { Hccl::AicpuDaemonService::GetInstance().ServiceStop(info); };
