    ...
    RankInfo& localRankInfo, RankInfo& remoteRankInfo, NicType nicType, s32 deviceLogicId)
{
    ...
    HcclIpAddress remoteIp =
        (nicType == NicType::DEVICE_NIC_TYPE) ? remoteRankInfo.nicIp[0] : remoteRankInfo.deviceVnicIp;
...
HcclResult DetectConnectionAnomalies::Connect(struct ErrInfo errInfo, std::shared_ptr<HcclSocket>& clientSocket)
{
    ...
    HcclIpAddress remoteIpAddr =
        (errInfo.nicType == NicType::VNIC_TYPE) ? errInfo.remoteRankInfo.deviceVnicIp : errInfo.remoteRankInfo.nicIp[0];
