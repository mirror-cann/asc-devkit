...
bool CpuTsThread::IsDeviceA5() const { return devType_ == DevType::DEV_TYPE_950; }
...
HcclResult CpuTsThread::LocalNotifyWait(uint32_t notifyIdx, uint32_t timeOut) const
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS, HCCL_ERROR("[%s]fail, notifyIdx[%u], timeOut[%u].", __func__, notifyIdx, timeOut), ret);
