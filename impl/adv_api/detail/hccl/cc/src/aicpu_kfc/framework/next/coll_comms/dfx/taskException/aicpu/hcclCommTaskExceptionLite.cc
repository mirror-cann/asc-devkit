...
HcclCommTaskExceptionLite::~HcclCommTaskExceptionLite() { initFlag_ = false; }
            ...
            if (cqeStatus != dfx::CqeStatus::kDefault) {
                ...
                CHK_PRT_RET(
                    ...
                    HCCL_ERROR(
                        ...
                        "cqeStatus[%d]",
                        __func__, aicpuComm->GetIdentifier().c_str(), streamLite->GetId(), cqeStatus),
                    ret);
            }
        }
    }
...
HcclResult HcclCommTaskExceptionLite::ProcessCqe(CollCommAicpu* aicpuComm, const rtLogicCqReport_t& exceptionInfo)
{
    ...
    if (curTask == nullptr) {
        // 未找到异常对应的TaskInfo
        HCCL_ERROR(
            "[%s]Exception task not found. devId_[%u], streamId(sqId)[%u], taskId(sqeId)[%u].", __func__, devId_,
            exceptionInfo.sqId, sqeId);
        return HCCL_E_PARA;
    }
    ...
    if (!aicpuComm->IsErrorReported()) {
        ...
        if (curTask->dfxOpInfo_ == nullptr) {
            HCCL_ERROR(
                "[%s]dfxOpInfo is nullptr. devId_[%u], streamId(sqId)[%u], taskId(sqeId)[%u].", __func__, devId_,
                exceptionInfo.sqId, sqeId);
        } else {
    ...
    // 1. 打印task信息
    HCCL_ERROR(
        "[TaskException][AICPU]base information is %s, %s", GetBaseInfo(*curTask).c_str(),
        curTask->GetParaInfo().c_str());
    // 2. UB任务打印EID信息
    PrintEid(*curTask);
    // 3. 打印group信息
...
std::string HcclCommTaskExceptionLite::GetBaseInfo(const Hccl::TaskInfo& taskInfo)
{
    ...
    return Hccl::StringFormat(
        "streamID(sqId):[%u], taskID(sqeId):[%u], taskType:[%s], opIndex[%u]", taskInfo.streamId_, taskInfo.taskId_,
        taskInfo.taskParam_.taskType.Describe().c_str(), opIndex);
}
    ...
    Hccl::ErrorMessageReport& errMsgInfo)
{
    ...
    strcpy_s(
        errMsgInfo.algType, MAX_NAME_LEN,
        taskInfo.dfxOpInfo_ == nullptr ? "MESH" : taskInfo.dfxOpInfo_->algType_.c_str());
    ...
    if (taskInfo.taskParam_.taskType == Hccl::TaskParamType::TASK_NOTIFY_WAIT) {
        ...
        taskInfo.taskParam_.taskType == Hccl::TaskParamType::TASK_UB_REDUCE_INLINE ||
...
void HcclCommTaskExceptionLite::GetErrMsgInfo(
    ...
    const u32 notifyId, const u32 tsId, const rtLogicCqReport_t& exceptionInfo)
{
    ...
    HCCL_RUN_INFO(
        "[%s]fininsh, notifyId=%u, hostpid=%u, vfId=%u, tsId=%u, errorType=%u, errorCode=%u, ret_code=%u", __func__,
...
HcclResult HcclCommTaskExceptionLite::PrintTaskContextInfo(u32 sqId, u32 taskId)
{
    ...
    CHK_PRT_RET(
        taskItorPtr == nullptr || *taskItorPtr == *queue->End(),
    ...
    for (uint32_t i = 0; i < TASK_CONTEXT_SIZE && *taskItorPtr != *queue->Begin(); ++i, --(*taskItorPtr)) {
        if ((**taskItorPtr)->taskId_ > taskId) {
            HCCL_ERROR(
                "[%s]prev taskId[%u] is bigger than err taskId[%u], stop traversal", __func__, (**taskItorPtr)->taskId_,
                taskId);
            break;
        }
        if ((**taskItorPtr)->taskId_ != taskId) {
    ...
    for (u32 i = 0; i < taskContext.size(); ++i) {
        ...
        if (taskContextInfo.size() + conciseInfo.size() >=
            ...
            HCCL_ERROR(
                "[TaskException][AICPU]task sequence is OP(%u): %s", lastTask->dfxOpInfo_->opIndex_,
                taskContextInfo.c_str());
            taskContextInfo = "";
...
std::string HcclCommTaskExceptionLite::GetGroupInfo(const Hccl::TaskInfo& taskInfo)
{
    ...
    return Hccl::StringFormat(
        "group:[%s], rankSize:[%u], localRank:[%d]", aicpuComm->GetIdentifier().c_str(),
...
std::string HcclCommTaskExceptionLite::GetOpDataInfo(const Hccl::TaskInfo& taskInfo)
{
    ...
    return Hccl::StringFormat(
        "opIndex[%u], algTag[%s], count[%llu], reduceType[%s], src[0x%llx], dst[0x%llx], dataType[%s]",
        opInfo->opIndex_, opInfo->algTag_.c_str(), opInfo->op_.dataCount, opInfo->op_.reduceOp.Describe().c_str(),
