    ...
    while (true) {
        ...
        if (iter->second->IsUsed()) {
            if ((std::chrono::steady_clock::now() - startTime) >= waitPollTimeOutMs) {
                HCCL_ERROR(
                    "[AicpuGetCommbyGroup][%s]poll timeout, comm group [%s] has been used", __func__, group.c_str());
                rwlock.readUnlock();
                return nullptr;
            }
...
ReadWriteLockBase& AicpuIndopProcess::AicpuGetCommMutex() { return g_commAicpuInfo.commAicpuMgrMapMutex; }
...
HcclResult AicpuIndopProcess::AicpuIndOpNotifyInit(NotifyMgrAicpuParam* param)
{
    ...
    if (param->freeFlag) {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[AicpuIndopProcess][%s]errNo[0x%016llx] Failed to free notifys group[%s]", __func__,
                HCCL_ERROR_CODE(ret), group.c_str()),
            ret);
    } else {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[AicpuIndopProcess][%s]errNo[0x%016llx] Failed to alloc notifys group[%s]", __func__,
                HCCL_ERROR_CODE(ret), group.c_str()),
            ret);
    }

    HCCL_INFO(
        "[AicpuIndopProcess][%s] comm identifier[%s], notify op[%u] success, num[%u]", __func__, group.c_str(),
...
HcclResult AicpuIndopProcess::ProfilingReportDeviceOp(const std::string& group)
{
    ...
    EXECEPTION_CATCH(
        Hccl::ProfilingHandlerLite::GetInstance().ReportHcclOpInfo(*mirrorTaskMgrLite->GetCurrDfxOpInfo()),
        return HCCL_E_INTERNAL);
    return HCCL_SUCCESS;
}
