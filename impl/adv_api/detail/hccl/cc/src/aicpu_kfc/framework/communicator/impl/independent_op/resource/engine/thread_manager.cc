...
HcclResult ThreadMgr::CheckNotifyNum(CommEngine engine, uint32_t threadNum, uint32_t notifyNumPerThread)
{
    ...
    HCCL_INFO(
        "[ThreadMgr][%s] Hcom[%s] HcclThreadAcquire quota: engine[%d], "
        "remainNotifyQuota[%u]",
        __func__, commId_.c_str(), engine, remainNotifyQuota);
    return HCCL_SUCCESS;
}
...
HcclResult ThreadMgr::CheckThreadNum(CommEngine engine, uint32_t threadNum, uint32_t notifyNumPerThread)
{
    ...
    HCCL_INFO(
        "[ThreadMgr][%s] Hcom[%s] HcclThreadAcquire quota: engine[%d] threadNum[%llu].", __func__, commId_.c_str(),
        engine, remainQuota);
...
HcclResult ThreadMgr::SupplementNotify(CommEngine engine, uint32_t threadNum, uint32_t notifyNumPerThread)
{
    ...
    HCCL_INFO(
        "[ThreadMgr][%s] Hcom[%s] threadNum[%zu] supplementNotifyNum[%u] notifyLoadType[%u]", __func__, commId_.c_str(),
        threads.size(), notifyNumPerThread, static_cast<int32_t>(notifyLoadType));

    DevType devType = DevType::DEV_TYPE_COUNT;
    CHK_RET(hrtGetDeviceType(devType));
    u32 aicpuHostNotify =
        (devType == DevType::DEV_TYPE_950 && (engine == COMM_ENGINE_AICPU_TS || engine == COMM_ENGINE_AICPU)) ? 1 : 0;

    for (uint32_t i = 0; i < threads.size() && i < threadNum; ++i) {
        HCCL_INFO(
            "[ThreadMgr][%s] Hcom[%s] AicpuTsThread notifyLoadType[%u]", __func__, commId_.c_str(),
        ...
        if (notifyNumPerThread > notifyNum) {
            ...
            if (ret != HCCL_SUCCESS) {
                HCCL_ERROR(
                    "[ThreadMgr][SupplementNotify] Failed to supplement notify[%u] on thread index %u",
...
HcclResult ThreadMgr::SupplementThread(CommEngine engine, uint32_t supplementThreadNum, uint32_t notifyNumPerThread)
{
    ...
    HCCL_INFO(
        "[ThreadMgr][%s] Hcom[%s] supplementThreadNum[%u] notifyNum[%u] notifyLoadType[%u], streamType[%u]", __func__,
        commId_.c_str(), supplementThreadNum, notifyNumPerThread, static_cast<int32_t>(notifyLoadType),
    ...
    for (uint32_t i = 0; i < supplementThreadNum; ++i) {
        ...
        HCCL_INFO(
            "[ThreadMgr][%s] Hcom[%s] notifyLoadType[%u], streamType[%u]", __func__, commId_.c_str(),
    ...
    if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
        ...
        EXECEPTION_CATCH(hostHandle = std::make_unique<ThreadHandle[]>(newThreads.size()), return HCCL_E_PTR);
    ...
    std::vector<uint32_t>& threadId)
{
    ...
    for (u32 idx = 0; idx < threadNum; idx++) {
        ...
        threads[idx] = (engine == COMM_ENGINE_AICPU_TS || engine == COMM_ENGINE_AICPU) ?
                           hostToDeviceThreadHandle_[handle] :
                           handle;
    ...
    HCCL_INFO(
        "[ThreadMgr][%s] Hcom[%s] HcclThreadAcquire done: engine[%d] threadNum[%u],"
        "notifyPerThread[%u]%s",
        __func__, commId_.c_str(), engine, threadNum, notifyNumPerThread,
    ...
    std::vector<uint32_t>& threadId)
{
    ...
    for (uint32_t i = 0; i < threadNum; ++i) {
        std::shared_ptr<Thread> handle;
        HCCL_INFO(
            "[ThreadMgr][%s] Hcom[%s] AicpuTsThread notifyLoadType[%u], streamType[%u]", __func__, commId_.c_str(),
            static_cast<int32_t>(notifyLoadType), static_cast<int32_t>(streamType));
        CHK_RET(CreateThread(engine, streamType, notifyNumPerThread, notifyLoadType, handle));
        ret = handle->Init();
        if (ret != HCCL_SUCCESS) {
    ...
    HCCL_INFO(
        "[ThreadMgr][HcclThreadAcquire] Hcom[%s] HcclThreadAcquire done: engine[%d] threadNum[%u],"
        "notifyPerThread[%u]%s",
        commId_.c_str(), engine, threadNum, notifyNumPerThread,
        (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) ? " (AICPU token ready)" : "");
    return HCCL_SUCCESS;
}
...
HcclResult ThreadMgr::HcclThreadAcquireWithStream(
    CommEngine engine, rtStream_t stream, uint32_t notifyNum, ThreadHandle* thread)
{
    ...
    *thread = reinterpret_cast<ThreadHandle>(mainThread_[stream].get());
    ...
    HCCL_INFO(
        "[ThreadMgr] Hcom[%s] HcclThreadAcquireWithStream done: engine[%d] stream[%p],"
        "notifyNum[%u]",
        commId_.c_str(), engine, stream, notifyNum);
    return HCCL_SUCCESS;
}

HcclResult ThreadMgr::ThreadExportToCommEngineCpu(
    uint32_t threadNum, const ThreadHandle* threads, ThreadHandle* exportedThreads)
{
    ...
    uint32_t threadNum, const ThreadHandle* threads, CommEngine dstCommEngine, ThreadHandle* exportedThreads)
{
    ...
    if (!hostThreads.empty()) {
        ...
        EXECEPTION_CATCH(aicpuHandle = std::make_unique<ThreadHandle[]>(hostThreads.size()), return HCCL_E_PTR);
