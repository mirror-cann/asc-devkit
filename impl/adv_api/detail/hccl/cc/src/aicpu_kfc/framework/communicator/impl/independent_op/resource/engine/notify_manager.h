...
public:
    ...
    inline LocalNotify* GetNotify(u32 index) const
    {
        if (index > notifyNum_) {
            HCCL_ERROR(
                "[NotifyManager][GetNotify]notifyNum[%u], notifyIdx[%u] out of range[0, %u]", notifyNum_, index,
