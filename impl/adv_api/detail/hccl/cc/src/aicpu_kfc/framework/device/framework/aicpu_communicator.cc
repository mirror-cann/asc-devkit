...
template <typename T>
HcclResult HcclCommAicpu::InitAndVerifySignal(
    const HcclSignalInfo& signalInfo, std::vector<std::shared_ptr<T>>& notifyVec)
{
        ...
        for (auto& singleSubCommTransport : levelNSubCommTransport) {
            ...
            for (auto& transportRequest : singleSubCommTransport.transportRequests) {
                ...
                if (transportRequest.isValid) {
                    ...
                    bool isBackup =
                        remoteRankPortMap.find(transportRequest.remoteUserRank) != remoteRankPortMap.end() &&
...
{
    HCCL_INFO(
        "[HcclCommAicpu][AllocScratchMemResource]requesting for [%u] bytes scratch mem, tag[%s].", scratchMemSize,
        newTag.c_str());
    if (scratchMemSize != 0) {
        ...
        if (tagScratchMem_.find(newTag) == tagScratchMem_.end()) {
            HCCL_ERROR(
                "[HcclCommAicpu][AllocScratchMemResource]alloc scratch memory failed. requesting for [%u] bytes,"
                " tag[%s].",
        ...
        if (reAllocFlag) {
            HCCL_INFO(
                "[HcclCommAicpu][AllocScratchMemResource]need to reAlloc mem [%u] bytes scratch mem, tag[%s].",
                scratchMemSize, newTag.c_str());
        ...
        if (scratchMemSize - tagScratchMem_[newTag]->size() > (CCE_REDUCE_ALIGN_SIZE + CCE_REDUCE_ALIGN_SIZE)) {
            HCCL_ERROR(
                ...
                "requesting [%u] bytes actual [%u] bytes",
    ...
    HCCL_INFO(
        "[HcclCommAicpu][AllocScratchMemResource]find enough [%u] bytes scratch mem, tag[%s].", scratchMemSize,
        newTag.c_str());
    return HCCL_SUCCESS;
}
...
HcclResult HcclCommAicpu::GetAlltoAllVCTotalCount(OpParam& param, u64& sendCount, u64& recvCount)
{
    for (u32 i = 0; i < topoInfo_.userRankSize; i++) {
        sendCount +=
            *(static_cast<const u64*>(param.All2AllDataDes.sendCountMatrix) +
    ...
    while (true) {
        switch (state) {
            ...
            case HcclOpExecFSM::HCCL_OP_EXEC_FSM_WAIT_END:
                ret =
                    HcclOpExecFsmWaitEndProcess(param, algResource, state, errorCode, retryCnt, param.tag, beginSqePos);
    ...
    uint32_t beginSqePos, uint32_t endSqePos)
{
    ...
    if (!HcclOpSupportRetry(algName, retryEnable_, param)) {
        ...
        if (param.isInplaceError) {
            ...
            CHK_PRT(SendTaskExceptionByMBox(rsErrorCode));
            HCCL_ERROR(
                "[OpRetry][AICPU][HcclOpExecFsmStoppedProcess]hccl aicpu exec fsm stop by zero copy op, "
    ...
    for (auto& stream : totalStream) {
        ...
        if (stream.id() == mainStream_.id()) {
            ...
            if (sqeInfo.type == RT_STARS_SQE_TYPE_NOTIFY_WAIT && sqeInfo.notifyId == opNotifies_[0]->notifyId_) {
                HCCL_RUN_INFO(
                    "[PrintTaskExceptionAllStreams] group[%s] op is not activated, do nothing", identifier_.c_str());
                return HCCL_SUCCESS;
            }
            // 根据主流当前位置，判断该算子是否已经打印过taskException
            if (IsRepeatedOpTaskException(sqHead, sqeContextBuffer)) {
                HCCL_INFO(
                    "[PrintTaskExceptionAllStreams] group[%s] op has been printed, do nothing", identifier_.c_str());
                return HCCL_SUCCESS;
            }
        }
    ...
    AlgResourceResponse& algResource, uint32_t& beginSqePos, uint32_t& endSqePos)
{
    ...
    HCCL_INFO(
        "[HcclCommAicpu][OrchestrateHcclOp] inputPtr[0x%016llx] inputSize[%u] outputPtr[0x%016llx] outputSize[%u]",
    ...
    auto setProfStartCallback = [this]() { return this->InvokeKfcHandler(AicpuKfcHandlerType::kSetProfTimeStart, {}); };
...
HcclResult HcclCommAicpu::GetAlltoAllvSendRecvInfo(
    const void* sendRecvInfoPtr, HcclDataType sendType, HcclDataType recvType)
{
...
HcclResult HcclCommAicpu::GetAlltoAllvcSendRecvInfo(
    const void* sendCountMatrix, HcclDataType sendType, HcclDataType recvType)
{
    allMeshAggregationSendRecvInfo_.clear();
    for (u32 i = 0; i < topoInfo_.userRankSize; i++) {
        SendRecvInfo sendRecvInfo;
...
void HcclCommAicpu::PrintTaskExceptionTaskQue(u32 sqIdx, SqeRingBuffer* sqeContextBuffer, bool isMonitor)
{
    ...
    for (u32 i = 0; i < sqeNum; i++) {
        ...
        if (sizeAfterAdd >= (maxTasksPerLine + 2) ||
...
HcclResult HcclCommAicpu::GetBsrTransportQpn(
    const HcclSendRecvItem* sendrecvPair, AlgResourceResponse& algResource, u32& qpn)
{
...
void HcclCommAicpu::HandleIndOpCqe()
{
    ...
    for (auto& thread : threads_) {
        ...
        // 打印重执行提示信息
        bool isSdmaTypeErr =
            cqeStatus == dfx::CqeStatus::kCqeException && cqeException.sqeType == RT_STARS_SQE_TYPE_SDMA;
        ...
        if (retryEnable_ && isSdmaCompDataErr) {
            ...
            CHK_PRT(SendTaskExceptionByMBox(rsErrorCode));
            HCCL_RUN_INFO(
                "[OpRetry][AICPU]group[%s] can not retry, IndOp does not support opRetry", identifier_.c_str());
        }
...
HcclResult HcclCommAicpu::RefreshRoceTransportsForSwitchNic(
...
HcclResult HcclCommAicpu::InitP2pChannel(HcclIndOpChannelRemoteResV3* commParam, uint32_t channelIndex)
{
    ...
    std::string channelKey = std::string(commParam->channelTag) + ":" + std::to_string(commParam->engine) + ":" +
                             ...
                             std::to_string(CommProtocol::COMM_PROTOCOL_HCCS);
