            ...
            if (superDeviceId_ != static_cast<u32>(drvSuperDeviceID)) {
                RPT_INPUT_ERR(
                    ...
                    std::vector<std::string>(
                        {"[" + std::to_string(superDeviceId_) + "]", "[super_device_id]",
                         std::to_string(drvSuperDeviceID)}));
                HCCL_ERROR(
...
HcclResult HcclCommunicatorAttrs::SetRanksPort(const std::vector<RankInfo_t>& rankList)
{
    ...
    isUseRankPort_ = ((devicePortSwitchOn && nicDeployment_ == NICDeployment::NIC_DEPLOYMENT_DEVICE) ||
                      isHaveCpuRank_ || nicDeployment_ == NICDeployment::NIC_DEPLOYMENT_HOST) ?
                         true :
                         isUseRankPort_;
    HCCL_INFO(
...
HcclResult HcclCommunicatorAttrs::InitRankInfoSubGroup(
