...
DevType MakeEnumToDevType(int makeEnum)
{
    ...
    static std::map<int, DevType> makeEnumToDevType = {{0, DevType::DEV_TYPE_910},    {1, DevType::DEV_TYPE_310P3},
                                                       {2, DevType::DEV_TYPE_910B},   {3, DevType::DEV_TYPE_310P1},
                                                       {4, DevType::DEV_TYPE_910_93}, {5, DevType::DEV_TYPE_950},
...
HcclResult HcomSetGradFusionByIndex(const char* group, u32 segmentNum, const u32* inputIdxList)
{
    ...
    RPT_INPUT_ERR(
        inputIdxList == nullptr, "EI0003", std::vector<std::string>({"ccl_op", "value", "parameter", "expect"}),
    ...
    CHK_PRT_RET(
        bRet,
        HCCL_ERROR(
            "[%s][%s]errNo[0x%016llx] set split inputIdxList length is zero", LOG_KEYWORDS_TASK_EXEC.c_str(),
            LOG_KEYWORDS_INVALID_ARGUMENT.c_str(), HCOM_ERROR_CODE(HCCL_E_PARA)),
        HCCL_E_PARA);
    std::string strGroup = (group == nullptr) ? HCCL_WORLD_GROUP : group;
    string idxList;
    ...
    /* 接口交互信息日志 */
    HCCL_RUN_INFO(
        "Entry-HcomSetGradFusionByIndex:group[%s], segmentNum[%u], inputIdxList[%s]", strGroup.c_str(), segmentNum,
        idxList.c_str());
...
HcclResult HcomNormalInit(const char* rankTableM, const char* identify)
{
    ...
    do {
        ...
        CHK_PRT_RET(
            hcomInfo.pComm == nullptr, HCCL_ERROR("[Init][Result]hcomInfo.pComm is null,\
            create failed"),
            HCCL_E_PTR);
