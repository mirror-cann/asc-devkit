    ...
    uint64_t count, char* algConfig, uint32_t commEngine, void** opResCtx)
{
    ...
    if (GetExternalInputHcclEnableEntryLog()) {
        ...
        std::string logInfo =
            "MC2 create resource by tiling: localRank[" + std::to_string(localRank) + "]" + std::string(stackLogBuffer);
    ...
    if (*opResCtx == nullptr) {
        HCCL_ERROR(
            "[%s] GetCommResource failed, opResCtx is nullptr, commIdentifier[%s]", __func__, commIdentifier.c_str());
