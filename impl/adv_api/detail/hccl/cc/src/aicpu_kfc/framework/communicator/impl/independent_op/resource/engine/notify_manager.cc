...
HcclResult NotifyManager::ParseBinNotifys(
    const std::string& uniqueIdStr, std::vector<std::unique_ptr<LocalNotify>>& newNotifys)
{
    ...
    HCCL_RUN_INFO(
        "[NotifyManager][%s] recover success, notifyNum[%u], notifyType[%u], uniqueIdSize[%s]", __func__, notifyNum,
        loadType, uniqueIdStr.size());
    return HCCL_SUCCESS;
}

#ifndef CCL_KERNEL_AICPU
std::string NotifyManager::GetBinNotifys(
    std::vector<std::unique_ptr<LocalNotify>>& newNotifys, const NotifyLoadType notifyType)
{
    ...
    HCCL_RUN_INFO(
        "[NotifyManager][%s] GetUniqueId success, notifyNum[%u], notifyType[%u], uniqueId[%s]", __func__, notifyNum,
...
HcclResult NotifyManager::HcommFreeNotify(uint32_t notifyNum, NotifyHandle* notifyHandleList)
{
    ...
    if (hasAicpu) {
        ...
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR(
                "[NotifyManager][%s] NotifyKernelLaunchFree failed ret[%d], num[%u], skip host erase", __func__, ret,
                aicpuNotifys.size());
    ...
    for (auto* localNotify : localNotifys) {
        ...
        auto it = std::find_if(
            notifys_.begin(), notifys_.end(),
            [localNotify](const std::unique_ptr<LocalNotify>& ptr) { return ptr.get() == localNotify; });
        if (it != notifys_.end()) {
            notifys_.erase(it);
        }
    }
    ...
    auto itBlock = std::find_if(
        handleBlocks_.begin(), handleBlocks_.end(),
        [notifyHandleList](const std::unique_ptr<NotifyHandle[]>& block) { return block.get() == notifyHandleList; });
