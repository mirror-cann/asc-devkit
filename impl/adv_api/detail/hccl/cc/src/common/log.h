...
#define HCCL_ERROR_LOG_PRINT(format, ...)                                                                            \
    do {                                                                                                             \
        if (IsErrorToWarn()) {                                                                                       \
            LOG_FUNC(                                                                                                \
                ASCENDC_MC2_RUN_LOG_MASK, DLOG_WARN, "[%s:%d] [%u]ErrToWarn: " format, __FILE__, __LINE__,           \
                syscall(SYS_gettid), ##__VA_ARGS__);                                                                 \
        } else {                                                                                                     \
            LOG_FUNC(                                                                                                \
                ASCENDC_MC2_DLOG_MODULE, DLOG_ERROR, "[%s:%d] [%u]" format, __FILE__, __LINE__, syscall(SYS_gettid), \
                ##__VA_ARGS__);                                                                                      \
        }                                                                                                            \
    } while (0)
...
#define HCCL_RUN_LOG_PRINT(format, ...)                                                                          \
    do {                                                                                                         \
        LOG_FUNC(                                                                                                \
            ASCENDC_MC2_RUN_LOG_MASK, DLOG_INFO, "[%s:%d] [%u]" format, __FILE__, __LINE__, syscall(SYS_gettid), \
            ##__VA_ARGS__);                                                                                      \
    } while (0)
