...
private:
    ...
    __aicore__ inline static void BatchDataCopyImpl(const T& dst, const U& src)
    {
        ...
        uint16_t ndNum = Get<0>(srcLayout.Shape());
        uint64_t srcNdMatrixStride = Get<0>(srcLayout.Stride());
        uint32_t dstNzMatrixStride = Get<0>(dstLayout.Stride());

        EmitCopy(dst, src, Te::Get<1>(srcLayout), Te::Get<1>(dstLayout), ndNum, srcNdMatrixStride,
                 dstNzMatrixStride);
    }
};
