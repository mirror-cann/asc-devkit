...
#pragma message( \
    "impl/basic_api/dav_m510/kfc/kfc_comm_client.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_operator_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
public:
    __aicore__ inline KfcCommClient(
        GM_ADDR workspace, int subBlockID, int8_t enableHardPoll = 0, int8_t enableMixDualMaster = 0);
    __aicore__ inline ~KfcCommClient();
    __aicore__ inline GM_ADDR AllocUB(uint32_t size, int32_t& tailInfo) { return nullptr; }
...
__aicore__ inline KfcCommClient::KfcCommClient(
    GM_ADDR workspace, int subBlockID, int8_t enableHardPoll, int8_t enableMixDualMaster)
{
    if ASCEND_IS_AIV {
