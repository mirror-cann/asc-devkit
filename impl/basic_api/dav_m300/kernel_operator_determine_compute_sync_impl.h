...
#pragma message( \
    "impl/basic_api/dav_m300/kernel_operator_determine_compute_sync_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_tensor.h\"\" and use public functions or variables defined in interface headers files.")
...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported InitDetermineComputeWorkspace on current device"); });
}

__aicore__ inline void WaitPreBlockCalc(const GlobalTensor<int32_t>& gmWorkspace, LocalTensor<int32_t>& ubWorkspace)
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported WaitPreBlock on current device"); });
}

__aicore__ inline void NotifyNextBlockCalc(GlobalTensor<int32_t>& gmWorkspace, LocalTensor<int32_t>& ubWorkspace)
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported NotifyNextBlock on current device"); });
}
} // namespace AscendC
