...
#pragma message( \
    "impl/basic_api/dav_c220/kernel_operator_vec_gather_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_vec_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void GatherbImpl(
    __ubuf__ T* dst, __ubuf__ T* src, __ubuf__ uint32_t* offset, const uint32_t srcLength, uint8_t repeatTime,
    const GatherRepeatParams& repeatParams)
{
    if ASCEND_IS_AIV {
        ASCENDC_ASSERT((SupportType<T, uint16_t, uint32_t>()), {
            KERNEL_LOG(
    ...
    const uint32_t srcBaseOffset, const uint64_t mask, const uint8_t repeatTime, const uint16_t dstRepStride)
{
    if ASCEND_IS_AIV {
        ASCENDC_DEBUG_ASSERT(
            ...
            KERNEL_LOG_INTERNAL(
                KERNEL_ERROR,
                "Failed to check dtype in Gather, current api support dtype combination "
    ...
    const uint32_t srcBaseOffset, const uint64_t mask[], const uint8_t repeatTime, const uint16_t dstRepStride)
{
    if ASCEND_IS_AIV {
        ...
        if constexpr (sizeof(T) == sizeof(uint16_t)) {
            vgather(
                (__ubuf__ uint16_t*)dstLocal, (__ubuf__ uint32_t*)srcOffsetLocal, offsetAddr, dstRepStride, repeatTime);
        } else if constexpr (sizeof(T) == sizeof(uint32_t)) {
            vgather(
                (__ubuf__ uint32_t*)dstLocal, (__ubuf__ uint32_t*)srcOffsetLocal, offsetAddr, dstRepStride, repeatTime);
        }
    }
}
} // namespace AscendC
