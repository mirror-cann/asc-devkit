...
namespace AscendC {
template <typename T>
__simd_vf__ inline void BilinearInterpolationCalc(
...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported BilinearInterpolation"); });
}

template <typename T>
__simd_vf__ inline void BilinearInterpolationCalc(
    LocalTensor<T>& dst, LocalTensor<T>& src0, LocalTensor<uint32_t>& src0Offset, LocalTensor<T>& src1,
    uint64_t mask[2], uint8_t hRepeat, bool repeatMode, uint16_t dstBlkStride, uint16_t vROffset, uint8_t vRepeat,
