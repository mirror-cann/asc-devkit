...
#pragma message( \
    "impl/basic_api/dav_m200/kernel_operator_cache_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_tensor.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void DataCachePreloadImpl(const GlobalTensor<uint64_t>& src, const T cacheOffset)
{
    if constexpr ((IsSameType<T, int16_t>::value) || (IsSameType<T, int64_t>::value)) {
        ...
        ASCENDC_ASSERT(
            false, { KERNEL_LOG(KERNEL_ERROR, "cacheOffset only supporte int16_t and int64_t on current device"); });
    }
}
...
__aicore__ inline void PreLoadImpl(void* pc, const int64_t preFetchLen) { preload(pc); }
