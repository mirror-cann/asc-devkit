...
#pragma message( \
    "impl/basic_api/dav_m200/kernel_operator_vec_gather_mask_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_vec_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void GatherMaskImpl(
    __ubuf__ uint16_t* dst, __ubuf__ uint16_t* src0, __ubuf__ uint16_t* src1, const uint8_t patternMode,
    const bool reduceMode, const uint32_t mask, const GatherMaskParams& gatherMaskParams, uint64_t& rsvdCnt)
{
    ...
    vreduce(
        dst, src0, src1, gatherMaskParams.repeatTimes, 1, gatherMaskParams.src0BlockStride, patternMode, 8,
...
__aicore__ inline void GatherMaskImpl(
    __ubuf__ uint32_t* dst, __ubuf__ uint32_t* src0, __ubuf__ uint32_t* src1, const uint8_t patternMode,
    const bool reduceMode, const uint32_t mask, const GatherMaskParams& gatherMaskParams, uint64_t& rsvdCnt)
{
    if (reduceMode) {
        SetMaskCount();
    ...
    const GatherMaskParams& gatherMaskParams, uint64_t& rsvdCnt)
{
    ...
    if constexpr (SupportType<U, uint16_t>()) {
        ASCENDC_ASSERT((SupportType<T, half, uint16_t, int16_t>()), {
            KERNEL_LOG(
                KERNEL_ERROR, "Failed to check dtype "
                              "in GatherMask when src1Pattern is uint16_t tensor, current api support dtype "
                              "combination is src and dst "
                              "both: half / uint16_t / int16_t.");
        });
    } else {
        ASCENDC_ASSERT((SupportType<T, float, uint32_t, int32_t>()), {
            KERNEL_LOG(
                KERNEL_ERROR, "Failed to check dtype "
                              "in GatherMask when src1Pattern is uint32_t tensor, current api support dtype "
                              "combination is src and dst "
