...
/* !
 * \file data_utils.h
 * \brief File input and output helpers.
 */
...
bool ReadFile(const std::string& file_path, void* buffer, size_t expected_size)
{
    ...
    if (stat(file_path.data(), &file_stat) == -1) {
        ERROR_LOG("Failed to get file status. path = %s", file_path.c_str());
        return false;
    }
    ...
    if (file_stat.st_size <= 0 || static_cast<uintmax_t>(file_stat.st_size) != static_cast<uintmax_t>(expected_size)) {
        ERROR_LOG(
