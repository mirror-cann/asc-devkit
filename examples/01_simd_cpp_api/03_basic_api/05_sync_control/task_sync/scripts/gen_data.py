...
def gen_golden_data_simple():
    ...
    input_x = np.random.uniform(min_val / 3, max_val / 3, input_shape).astype(
        input_type
    )
    input_y = np.random.uniform(min_val / 3, max_val / 3, input_shape).astype(
        input_type
    )
    golden = np.add(input_x.astype(np.float32), input_y.astype(np.float32)).astype(
        output_type
    )
