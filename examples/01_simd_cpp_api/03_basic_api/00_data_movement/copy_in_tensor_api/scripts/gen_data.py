...
def gen_golden_data(scenario_num):
    ...
    # SCENARIO_NUM: 0-AB不转置half, 1-AB不转置float, 2-AB转置half, 3-AB转置float
    is_transpose = scenario_num >= 2
    dtype = np.float32 if scenario_num % 2 == 1 else np.half
    ...
    if is_transpose:
        # 转置存储: A^T=[K,M], B^T=[N,K]
        input_x.transpose().tofile("./input/input_x.bin")
...
if __name__ == "__main__":
    scenario_num = int(sys.argv[1]) if len(sys.argv) > 1 else 0
