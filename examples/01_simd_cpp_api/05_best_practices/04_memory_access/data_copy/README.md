...
**实现方式**：重复搬运场景在同一数据规模下执行多轮 GM 读取，使用 `msopprof --ai-core=on --aic-metrics=L2Cache` 采集 L2Cache 读命中和 miss allocate 数据。
...
- 性能采集
  ...
  ```bash
  msopprof ./demo
  msopprof --ai-core=on --aic-metrics=L2Cache ./demo    # L2Cache 相关场景使用
  ```
