...
- 样例执行
  ...
  cmake -DSCENARIO_NUM=$SCENARIO ..;make -j;    # 编译工程，默认npu模式
  ...
  示例如下：
  ```bash
  cmake -DCMAKE_ASC_RUN_MODE=cpu -DSCENARIO_NUM=$SCENARIO ..;make -j; # CPU仿真模式
  cmake -DCMAKE_ASC_RUN_MODE=sim -DSCENARIO_NUM=$SCENARIO ..;make -j; # NPU仿真模式
  ```
