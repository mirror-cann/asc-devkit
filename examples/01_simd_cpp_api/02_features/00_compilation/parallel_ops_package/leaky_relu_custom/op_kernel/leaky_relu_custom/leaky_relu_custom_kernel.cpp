...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t* x, __gm__ uint8_t* y, uint32_t totalLength, uint32_t tileNum, float negativeSlope, AscendC::TPipe* pipeIn)
    {
        ...
        this->pipe->InitBuffer(inQueueX, BUFFER_NUM, this->tileLength * sizeof(float));
        this->pipe->InitBuffer(outQueueY, BUFFER_NUM, this->tileLength * sizeof(float));
        this->pipe->InitBuffer(tmpBuffer1, this->tileLength * sizeof(float));
        this->pipe->InitBuffer(tmpBuffer2, this->tileLength * sizeof(float));
    }
...
extern "C" __global__ __aicore__ void leaky_relu_custom(__gm__ uint8_t* x, __gm__ uint8_t* y, __gm__ uint8_t* workspace, __gm__ uint8_t* tiling)
{
    ...
    AscendC::TPipe pipe;
    KernelLeakyRelu op;
    op.Init(x, y, tiling_data.totalLength, tiling_data.tileNum, tiling_data.negativeSlope, &pipe);
    op.Process();
}
