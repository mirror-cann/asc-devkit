...
def gen_golden_data(scenarioNum=1):
    ...
    bias_gm = np.random.uniform(-1, 3, [N]).astype(np.float32)
    ...
    if scenarioNum == 1:
        block_cols = 16
        x1_gm = x1_gm.reshape((int(M / 16), 16, int(K / block_cols), block_cols)).transpose(2, 0, 1, 3).astype(input_type)
        x2_gm = x2_gm.reshape((int(K / 16), 16, int(N / block_cols), block_cols)).transpose(2, 0, 1, 3).astype(input_type)
    elif scenarioNum == 2:
        pass
    elif scenarioNum == 3:
