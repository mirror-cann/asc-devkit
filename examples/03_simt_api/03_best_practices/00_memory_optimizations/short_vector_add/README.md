...
- 性能分析：Add算子进行简单的X+Y=Z，在回写结果时命中率低于50%，说明写操作被阻塞超过50%，触发额外的内存回读，被判定位写未命中。一般原因为数据未对齐或者数据类型小导致Warp不能填满。
...
使用`half2`类型的两组输入数据替代原有`half`类型数据，需要处理元素个数位奇数场景的单个`half`数据。
