# 基于AscenC 基础API的MxFP4矩阵乘法多场景示例

## 概述

本样例基于 Ascend C 基础 API 和静态 Tensor 编程方式，实现低比特浮点数的 MxFP4 矩阵乘法。通过编译参数 `SCENARIO_NUM` 选择不同场景。
...
│   ├── data_utils.h                       // 数据读写工具
│   ├── scripts
│   │   ├── gen_data.py                    // 输入数据和真值生成脚本
│   │   └── verify_result.py               // 结果校验脚本
│   └── matmul_mxfp4_basic_api.asc   // Ascend C 样例实现
```

## 样例规格

本样例实现固定 shape 为 `7168 x 7168 x 7168` 的 MxFP4 矩阵乘法，输出数据类型为 `bfloat16_t`。
...
每个核负责一个 `singleCoreM * singleCoreN` 子块，在 M/N 方向循环、在 K 方向按 `baseK` 累加。A/B 和 scaleA/scaleB 先从 GM 搬到 L1，再加载到 L0A/L0B，完成 Cube 累加计算，最后通过 `Fixpipe` 写回 GM。
...
| `stepKa` |  | GM->L1搬运中，A 在 K 方向的搬运步长，用于控制 GM->L1 的 K 方向搬运粒度|
| `stepKb` |  | GM->L1搬运中，B 在 K 方向的搬运步长，用于控制 GM->L1 的 K 方向搬运粒度|
| `scaleFactorKa` |  | GM->L1搬运中，scaleA 相对 A 的 K 方向搬运比例 |
| `scaleFactorKb` |  | GM->L1搬运中，scaleB 相对 B 的 K 方向搬运比例 |
| `usedCoreNum` |  | 使用核数 |

- 部分参数如下图所示：
  <img src="figure/Tiling.png">

本样例通过两组不同的tiling参数演示不同的性能：
- 场景1：M=K=N=7168，使用16核，单核处理 1792 x 7168 x 1792，baseM/K/N 取 128，stepK取1，scaleFactorKa取1
- 场景2：M=K=N=7168，使用16核，单核处理 1792 x 7168 x 1792，baseM/K/N 取 256，stepK取2，scaleFactorKa取1
