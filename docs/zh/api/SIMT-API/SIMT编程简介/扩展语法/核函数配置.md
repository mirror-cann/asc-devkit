...
执行配置由4个参数决定，详细用法说明参考[核函数配置](https://gitcode.com/cann/asc-devkit/blob/master/docs/zh/guide/编程指南/语言扩展层/SIMT-BuiltIn关键字.md#核函数配置)：
...
-   dyn\_ubuf\_size：size\_t类型，用于指定每个线程块动态分配的共享内存大小，单位为字节。这部分内存供数组使用，具体用法请参考[共享内存](https://gitcode.com/cann/asc-devkit/blob/master/docs/zh/guide/编程指南/编程模型/AI-Core-SIMT编程/内存层级.md#共享内存)中的“动态申请”方式。
