...
| valueType | 输入 | 输入、输出Value的数据类型，参数类型为[AscendC::TensorDataType](../数据结构/TensorDataType.md)。与Sort接口的模板参数T保持一致。 |
| indexType | 输入 | 输入、输出Index的数据类型，参数类型为[AscendC::TensorDataType](../数据结构/TensorDataType.md)。与Sort接口的模板参数U保持一致。 |
