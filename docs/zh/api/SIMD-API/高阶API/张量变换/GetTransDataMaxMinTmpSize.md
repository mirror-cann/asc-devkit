...
| srcShape | 输入 | 输入源操作数的shape大小，参数类型为[AscendC::TensorShape](../数据结构/TensorShape.md)，参数取值与TransData接口的params.srcLayout参数中的shape信息保持一致。 |
| dstShape | 输入 | 输出目的操作数的shape大小，参数类型为[AscendC::TensorShape](../数据结构/TensorShape.md)，参数取值与TransData接口的params.dstLayout参数中的shape信息保持一致。 |
| dataType | 输入 | 输入的数据类型，参数类型为[AscendC::TensorDataType](../数据结构/TensorDataType.md)，当前只支持half/float/uint16_t/int16_t数据类型的输入。 |
