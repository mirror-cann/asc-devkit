...
-   算子核函数实现
    ...
    当前Tiling下沉仅支持融合算子，为了模拟融合算子场景，通过[KERNEL\_TASK\_TYPE\_DEFAULT](https://gitcode.com/cann/asc-devkit/blob/master/docs/zh/api/SIMD-API/基础API/Kernel-Tiling/设置Kernel类型.md)接口强制指定算子在AIC、AIV混合场景运行。
