...
| 参数名 | 输入/输出 | 描述 |
| :--- | :--- | :--- |
| pipe | 输入 | 阻塞的流水类别。不支持PIPE_S。如果不关注流水类别，希望阻塞所有流水，可以传入PIPE_ALL，传入PIPE_ALL时等同于[asc_sync](asc_sync.md)。<cann-filter npu_type="950">Ascend 950PR/Ascend 950DT产品不支持传入PIPE_V。</cann-filter> |
