...
| --- | --- | --- |
| [SIMD API](SIMD-API/SIMD-API列表.md) | [基础API](SIMD-API/基础API/基础API.md) | 实现对硬件能力的抽象，开放芯片的能力，保证完备性和兼容性。标注为ISASI（Instruction Set Architecture Special Interface，硬件体系结构相关的接口）类别的API，不能保证跨硬件版本兼容。 |
| [SIMD API](SIMD-API/SIMD-API列表.md) | [C API](SIMD-API/C-API/C-API.md) | 纯C接口，开放芯片完备编程能力，支持数组分配内存，一般基于指针编程，提供与业界一致的C语言编程体验。 |
| [SIMD API](SIMD-API/SIMD-API列表.md) | [高阶 API](SIMD-API/高阶API/高阶API.md) | 实现一些常用的计算算法，用于提高编程开发效率，通常会调用多种基础API实现。高阶API包括数学库、Matmul、Softmax等API。高阶API可以保证兼容性。 |
