...
|参数名称|含义|
|----------|----------|
|T|源操作数和目的操作数的数据类型。<br>&bull; **Load2DMX接口**<br>Ascend 950PR/Ascend 950DT，支持数据类型为：fp4x2_e2m1_t、fp4x2_e1m2_t、fp8_e5m2_t、fp8_e4m3fn_t。|
|U|&bull; 针对Load2DMX接口，U用来表示dst的数据类型，当src为fp8_e5m2_t、fp8_e4m3fn_t时，U需为T对应的MX数据类型，即AscendC::mx_fp8_e5m2_t和AscendC::mx_fp8_e4m3_t，否则编译失败。除此之外的数据类型要求T和U一致。|
