...
|dstGap|相邻迭代间，目的操作数前一个分形**结束地址**与后一个分形**起始地址**的间隔，单位：512字节。取值范围：dstGap∈[0, 65535]。默认为0。<br>**注：dstGap = 0表示相邻repeat目的操作数起始地址间隔1个数据分形即连续存放。**<cann-filter npu_type="910"><br>注：Atlas 训练系列产品此参数无效。</cann-filter>|
