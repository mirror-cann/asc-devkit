...
用于设置[LoadDataWithStride](../矩阵数据搬入至L0-Buffer/LoadDataWithStride.md)接口的repeat参数。设置repeat参数后，可以通过调用一次LoadDataWithStride接口完成多个迭代的数据搬运，还用于配置输出矩阵K轴方向偏移。
...
| 参数名称 | 输入/输出 | 含义 |
| --- | --- | --- |
| repeatParams | 输入 | 设置[LoadDataWithStride](../矩阵数据搬入至L0-Buffer/LoadDataWithStride.md)接口的repeat参数，类型为SetLoadDataRepeatWithStride。<br>具体定义请参考：\$\{INSTALL_DIR\}/include/ascendc/basic_api/interface/kernel_struct_mm.h，\$\{INSTALL_DIR\}请替换为CANN软件安装后文件存储路径。<br>参数说明请参考[表2](#table15780447181826)。 |
...
- 调用[LoadDataWithStride](../矩阵数据搬入至L0-Buffer/LoadDataWithStride.md)接口时，必须配置本接口中的dstStride参数，若不使能repeat模式，repeatTime、repeatStride、repeatMode使用默认构造值。
