...
**图 1**  三类Cache在AI Core中的位置关系示意图<a name="zh-cn_topic_0000002583420201_fig496311278412"></a>  
...
当向GM写入数据，并且使能Cache时，存在以下两种Cache写策略：延迟写回（Write-Back）和直写（Write-Through）。
...
| Cache类型 | 接口名称 | 功能简述 |
|-----------|---------|---------|
| L2 Cache | [SetL2CacheHint](../../基础数据结构/GlobalTensor/SetL2CacheHint.md) | 读写GM的数据都默认被缓存在L2 Cache（默认使能L2 Cache），SetL2CacheHint能够设置GlobalTensor是否使能L2 Cache。 |
