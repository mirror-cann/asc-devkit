...
一个AI Core对应1个AIC和M个AIV。N个AI Core被启动，此时即有N个AIC和M \* N个AIV启动。
...
- N个AI Core中的M \* N个AIV进行全核同步。
    ...
    AscendC::CrossCoreSetFlag<0, PIPE_M>(0);       // 待前置PIPE_M流水任务完成后，通知调度器
    AscendC::CrossCoreWaitFlag(0);                 // 直到所有AIC都完成前置PIPE_M流水，调度器更新flagId=0的计数器后，解除阻塞。
    ```
...
AIC 2的CrossCoreSetFlag执行完后，此时调度器感知到2个AIC均已执行完CrossCoreSetFlag，因此所有AIC各自的flagId=0的计数器值增加为1。AIC 1和AIC 2检测到各自对应的flagId=0的计数器变为1，都解除阻塞，继续执行后续指令，并且将计数器值减去1。

**图 1**  多AI Core中AIC全核同步<a name="zh-cn_topic_0000002517901268_fig288716464319"></a>  
...
AscendC::CrossCoreSetFlag<1, PIPE_V>(0);       // 待前置PIPE_V流水任务完成后，通知调度器
AscendC::CrossCoreWaitFlag(0);                 // 直到该AI Core中的所有AIV都完成前置PIPE_V流水，调度器更新flagId=0的计数器后，解除阻塞。
```
...
AIV 1-2的CrossCoreSetFlag执行完后，此时调度器感知到2个AIV均已执行完CrossCoreSetFlag，因此将AIV 1-1和AIV 1-2各自的flagId=0的计数器值增加为1。AIV 1-1和AIV 1-2检测到各自对应的flagId=0的计数器变为1，都解除阻塞，继续执行后续指令，并且将计数器值减去1。

**图 2**  单AI Core中AIV全核同步<a name="zh-cn_topic_0000002518061186_fig15383112914156"></a>  
...
if ASCEND_IS_AIV {
    AscendC::CrossCoreSetFlag<2, PIPE_MTE3>(0);       // 待前置PIPE_MTE3流水任务完成后，通知调度器。必须要所有AIV都执行。
}
if ASCEND_IS_AIC {
    AscendC::CrossCoreWaitFlag(0);                    // 直到该AI Core中的所有AIV都完成前置PIPE_MTE3流水，调度器更新flagId=0的计数器后，AIC解除阻塞。
}
```
...
当AIV 1-1的Vector指令1、2全部执行完毕后，前置PIPE\_V的指令全部完成，CrossCoreSetFlag生效。此时调度器感知到2个AIV均已执行完CrossCoreSetFlag，因此将AIC 1 flagId=0的计数器值增加为1。AIC 1检测到对应的flagId=0的计数器变为1，则AIC 1核解除阻塞，继续执行后续指令Matrix指令2，并且将计数器值减去1。

**图 3**  单AI Core中AIC与AIV全核同步（AIV进行CrossCoreSetFlag）<a name="zh-cn_topic_0000002549581037_fig164171141103813"></a>  
