...
- 迭代变量类型<br>VF内所有Loop的迭代变量必须是uint16_t 类型。
...
- 连续对齐搬出(StoreAlign)
  ...
  如图[float转int16_t类型转换过程](#fig2)，将UB地址xAddr上数据量为VL/2的half类型元素通过LoadAlign（分布模式为DIST_UNPACK_B16）搬入xReg，调用Cast将half转为int32_t，写入yReg，最后通过StoreAlign（分布模式为DIST_NORM）搬出至UB地址yAddr。
