...
LoadAlign能够实现数据从Unified Buffer（UB）非连续搬运至[RegTensor](../概述/寄存器数据类型/RegTensor.md)，有两类接口：
...
| repeatStride | 输入 | 地址更新步长，单位：DataBlock（32B）。repeatStride * sizeof(T)需要32字节对齐。根据postMode的取值有两种情况：<br>&bull; POST_MODE_NORMAL：实际搬运的UB起始地址为dstAddr + repeatStride * 32B / sizeof(T)，搬运后地址不会更新。<br>&bull; POST_MODE_UPDATE：实际搬运UB起始地址为srcAddr，搬运后执行地址更新srcAddr +=  repeatStride * 32B / sizeof(T)。 |
