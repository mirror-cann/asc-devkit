...
| selMask | 输入 | 选取mask。<br>类型为[LocalTensor](../../../基础数据结构/LocalTensor/LocalTensor.md)，支持的TPosition为VECIN/VECCALC/VECOUT。<br>LocalTensor的起始地址需要32字节对齐。<br>每个比特位表示1个元素的选取，当selMask的比特位为1时，从src0中选取元素；比特位为0时，从src1中选取元素。<br>各模式下的selMask使用方式详见selMode参数说明。<br> |
...
本样例中只展示Compute流程中的部分代码。如果您需要运行样例代码，完整的调用样例可参考[Select样例](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/03_libraries/07_filter/select)。
