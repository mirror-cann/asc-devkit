...
| 参数名 | 输入/输出 | 描述 |
| :----- | :-------- | :--- |
| dst | 输入/输出 | 目的操作数。<br>类型为[LocalTensor](../../../基础数据结构/LocalTensor/LocalTensor.md)，支持的TPosition为VECIN/VECCALC/VECOUT。 |
...
<cann-filter npu-type="950">Ascend 950PR/Ascend 950DT，支持的数据类型为：half、bfloat16_t、float、int64_t、uint64_t。</cann-filter>
