...
| [asc_get_mask_spr](reg/addr_reg_compute/asc_get_mask_spr.md) | 从特殊寄存器SPR{MASK1, MASK0}读取mask值并根据数据类型格式返回对应的mask数据，MASK0、MASK1均为64bit的寄存器。 |
...
| [asc_copy](reg/addr_reg_compute/asc_copy.md) | 完成数据在寄存器内的搬运。 |
| [asc_create_addr_reg](reg/addr_reg_compute/asc_create_addr_reg.md) | 地址寄存器通过该接口初始化，然后在循环之中使用地址寄存器存储地址偏移量。 |
| [asc_create_iter_reg（废弃）](reg/addr_reg_compute/asc_create_iter_reg_deprecated.md) | 该接口已废弃，请使用asc_create_addr_reg实现此功能。 |
| [asc_create_mask](reg/addr_reg_compute/asc_create_mask.md) | 根据入参生成相应的掩码寄存器。 |
...
| [asc_update_mask](reg/addr_reg_compute/asc_update_mask.md) | 根据value大小生成对应的掩码寄存器中的值。 |
