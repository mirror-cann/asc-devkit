...
-   用户可以灵活自主地控制矩阵乘计算结果的搬运，例如将多次Iterate计算的矩阵乘结果缓存在CO1内存中，在需要搬出该结果时，一次性搬出多块baseM \* baseN的C矩阵。这种灵活搬运场景下，用户需要提前申请CO1的内存，调用[用户自主管理CO1](#li4843165185812)的Iterate函数原型后，一次Iterate的计算结果会输出到用户申请的CO1内存上。在需要搬出计算结果时，调用[Fixpipe](../../../基础API/矩阵计算（ISASI）/数据搬运/Fixpipe.md)接口搬运CO1上的结果，完成后释放申请的CO1内存。具体示例请参考[用户自主管理CO1的矩阵乘场景](Matmul使用说明.md#li12765161915318)。
...
<p id="p1656210291567"><a name="p1656210291567"></a><a name="p1656210291567"></a>针对<span id="ph1256272920563"><a name="ph1256272920563"></a><a name="ph1256272920563"></a><term id="zh-cn_topic_0000001312391781_term354143892110_2"><a name="zh-cn_topic_0000001312391781_term354143892110_2"></a><a name="zh-cn_topic_0000001312391781_term354143892110_2"></a>Atlas 200I/500 A2 推理产品</term></span>，该参数仅支持配置为false。</p>
</td>
</tr>
</tbody>
</table>
