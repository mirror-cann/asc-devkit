...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| tiling | 输入 | BatchMatmul Tiling的结构体，即BatchMatmulTiling对象得到的TCubeTiling结构体。<br><br>TCubeTiling为Kernel侧定义的Matmul TilingData，与入参为带AscendC::tiling命名空间的TCubeTiling结构体的[GetTiling](../Matmul-Tiling类/GetTiling.md)接口配合使用。 |
