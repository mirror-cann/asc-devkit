...
| isReuseSrc | 是否允许修改源操作数，默认值为false。<br>Ascend 950PR/Ascend 950DT，该参数仅在输入的数据类型为float时生效，取值如下：<br>true：开发者允许源操作数被改写，可以设置该参数取值为true开启，开启后本接口内部计算时复用srcTensor的内存空间，节省部分内存空间；<br>false：本接口内部计算时不复用srcTensor的内存空间。<br>Atlas A3 训练系列产品/Atlas A3 推理系列产品，该参数预留，传入默认值false即可。<br>Atlas A2 训练系列产品/Atlas A2 推理系列产品，该参数预留，传入默认值false即可。<br>Atlas 推理系列产品AI Core，该参数预留，传入默认值false即可。<br><!-- npu="x90" id9 -->Kirin X90，该参数预留，传入默认值false即可。<!-- end id9 --><br><!-- npu="9030" id10 -->Kirin 9030，该参数预留，传入默认值false即可。<!-- end id10 --> |
