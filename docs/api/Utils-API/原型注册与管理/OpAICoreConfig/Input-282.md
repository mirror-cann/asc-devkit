...
通过该接口，可针对不同的AI处理器型号[注册差异化的算子原型](https://gitcode.com/cann/asc-devkit/blob/9.1.0/docs/guide/编程指南/高级编程/Aclnn算子工程化开发/算子原型定义.md)。调用该接口后会返回一个OpParamDef结构，后续可通过该结构配置算子输入信息。
