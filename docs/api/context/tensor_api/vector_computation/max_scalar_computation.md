...
MaxScalar计算对源 Tensor 的每个元素与标量值执行取最大值运算，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::MaxScalar` 操作类型来实现。

计算公式为：`dst[i] = max(src[i], value)`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，MaxScalar 计算取 `Inst::MaxScalar`。 |
| TraitType | 计算特征参数类型，MaxScalar 计算使用默认值 `Std::ignore_t`。 |
...
__aicore__ inline void MaxScalarComputeExample(__gm__ half* gmX, __gm__ half* gmZ)
{
    ...
    Transform<Inst::MaxScalar>(zLocal, xLocal, half(2.0));
