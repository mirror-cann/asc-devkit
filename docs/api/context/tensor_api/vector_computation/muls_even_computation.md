...
MulsEven计算对源 Tensor 的偶数位元素与标量值执行乘法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::MulsEven` 操作类型来实现。

计算公式为：`dst[2i] = src[2i] * value`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，MulsEven 计算取 `Inst::MulsEven`。 |
| TraitType | 计算特征参数类型，MulsEven 计算使用默认值 `Std::ignore_t`。 |
...
- `Inst::MulsEven` 仅处理偶数位元素，通常与 `Inst::MulsOdd` 配合使用以覆盖完整 Tensor。
...
__aicore__ inline void MulsEvenComputeExample(__gm__ half* gmX, __gm__ half* gmZ)
{
    ...
    Transform<Inst::MulsEven>(zLocal, xLocal, half(2.0));
