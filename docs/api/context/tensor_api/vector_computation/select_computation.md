...
Select计算对两个源 Tensor 的对应元素执行逐元素按位选择，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::Select` 操作类型来实现。

该接口使用当前向量 mask 控制每个元素的有效写回；底层指令按位选择 `src0` 或 `src1` 的数据位生成结果。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Select 计算取 `Inst::Select`。 |
| TraitType | 计算特征参数类型，Select 计算使用默认值 `Std::ignore_t`。 |
...
| 操作类型 | 支持的数据类型 |
| :--- | :--- |
| `Inst::Select` | uint8_t, int8_t, uint16_t, int16_t, half, int32_t, uint32_t, float |
...
- 对 half、float 类型执行按位选择时，按底层数据位模式进行处理。
...
__aicore__ inline void SelectComputeExample(__gm__ uint16_t* gmX, __gm__ uint16_t* gmY, __gm__ uint16_t* gmZ)
{
    ...
    Transform<Inst::Select>(zLocal, xLocal, yLocal);
