...
Or计算对两个源 Tensor 的对应元素执行逐元素按位或运算，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::Or` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Or 计算取 `Inst::Or`。 |
| TraitType | 计算特征参数类型，Or 计算使用默认值 `Std::ignore_t`。 |
...
| 操作类型 | 支持的数据类型 |
| :--- | :--- |
| `Inst::Or` | uint8_t, int8_t, uint16_t, int16_t, half, int32_t, uint32_t, float |
...
__aicore__ inline void OrComputeExample(__gm__ uint16_t* gmX, __gm__ uint16_t* gmY, __gm__ uint16_t* gmZ)
{
    ...
    Transform<Inst::Or>(zLocal, xLocal, yLocal);
