...
ShiftLeftScalar计算对源 Tensor 的每个元素与标量值执行逻辑左移，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::ShiftLeftScalar` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，ShiftLeftScalar 计算取 `Inst::ShiftLeftScalar`。 |
| TraitType | 计算特征参数类型，ShiftLeftScalar 计算使用默认值 `Std::ignore_t`。 |
...
| 操作类型 | 支持的数据类型 |
| :--- | :--- |
| `Inst::ShiftLeftScalar` | uint8_t, int8_t, uint16_t, int16_t, half, int32_t, uint32_t, float |
...
__aicore__ inline void ShiftLeftScalarComputeExample(__gm__ uint16_t* gmX, __gm__ uint16_t* gmZ)
{
    ...
    Transform<Inst::ShiftLeftScalar>(zLocal, xLocal, uint16_t(2));
