...
Add计算对两个源 Tensor 的对应元素执行逐元素加法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::Add` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Add 计算取 `Inst::Add`。 |
| TraitType | 计算特征参数类型，Add 计算使用默认值 `Std::ignore_t`。 |
...
__aicore__ inline void AddComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
