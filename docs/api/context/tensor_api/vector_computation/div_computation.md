...
Div计算对两个源 Tensor 的对应元素执行逐元素除法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::Div` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Div 计算取 `Inst::Div`。 |
| TraitType | 计算特征参数类型，Div 计算使用默认值 `Std::ignore_t`。 |
| DstTensor | 目的张量类型，需为 Tensor API Tensor 类型，存储位置为 `Location::UB`。 |
| SrcTensor0 | 被除数张量类型，需为 Tensor API Tensor 类型，存储位置为 `Location::UB`。 |
| SrcTensor1 | 除数张量类型，需为 Tensor API Tensor 类型，存储位置为 `Location::UB`。 |
...
| src0 | 输入 | 被除数张量，存储位置为 `Location::UB`。 |
| src1 | 输入 | 除数张量，存储位置为 `Location::UB`。 |
...
__aicore__ inline void DivComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
    ...
    Transform<Inst::Div>(zLocal, xLocal, yLocal);
