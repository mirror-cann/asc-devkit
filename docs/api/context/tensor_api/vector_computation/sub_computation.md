...
Sub计算对两个源 Tensor 的对应元素执行逐元素减法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::Sub` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Sub 计算取 `Inst::Sub`。 |
| TraitType | 计算特征参数类型，Sub 计算使用默认值 `Std::ignore_t`。 |
...
| src0 | 输入 | 被减数张量，存储位置为 `Location::UB`。 |
| src1 | 输入 | 减数张量，存储位置为 `Location::UB`。 |
...
__aicore__ inline void SubComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
    ...
    Transform<Inst::Sub>(zLocal, xLocal, yLocal);
