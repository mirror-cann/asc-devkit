...
ExpSubOdd计算对奇数位元素执行指数减法运算，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::ExpSubOdd` 操作类型来实现。

计算公式为：`dst[2i+1] = e^(-src0[2i+1] + src1[2i+1])`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，ExpSubOdd 计算取 `Inst::ExpSubOdd`。 |
| TraitType | 计算特征参数类型，ExpSubOdd 计算使用默认值 `Std::ignore_t`。 |
...
- `Inst::ExpSubOdd` 仅处理奇数位元素，通常与 `Inst::ExpSubEven` 配合使用以覆盖完整 Tensor。
...
__aicore__ inline void ExpSubOddComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
    ...
    Transform<Inst::ExpSubOdd>(zLocal, xLocal, yLocal);
