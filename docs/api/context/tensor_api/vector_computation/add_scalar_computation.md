...
AddScalar计算对源Tensor的每个元素与标量值执行加法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::AddScalar` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，AddScalar 计算取 `Inst::AddScalar`。 |
| TraitType | 计算特征参数类型，AddScalar 计算使用默认值 `Std::ignore_t`。 |
...
- 源 Tensor 和目的 Tensor 的数据量（由 Layout 的Size()确定）需匹配。
...
__aicore__ inline void AddScalarComputeExample(__gm__ half* gmX, __gm__ half* gmZ)
{
    ...
    Transform<Inst::AddScalar>(zLocal, xLocal, half(2.0));
