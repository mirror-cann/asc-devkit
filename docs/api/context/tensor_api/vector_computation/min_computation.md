...
Min计算对两个源 Tensor 的对应元素执行逐元素取最小值运算，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::Min` 操作类型来实现。

计算公式为：`dst[i] = min(src0[i], src1[i])`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Min 计算取 `Inst::Min`。 |
| TraitType | 计算特征参数类型，Min 计算使用默认值 `Std::ignore_t`。 |
...
__aicore__ inline void MinComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
    ...
    Transform<Inst::Min>(zLocal, xLocal, yLocal);
