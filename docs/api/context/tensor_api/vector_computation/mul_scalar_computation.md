...
MulScalar计算对源 Tensor 的每个元素与标量值执行乘法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::MulScalar` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，MulScalar 计算取 `Inst::MulScalar`。 |
| TraitType | 计算特征参数类型，MulScalar 计算使用默认值 `Std::ignore_t`。 |
...
__aicore__ inline void MulScalarComputeExample(__gm__ half* gmX, __gm__ half* gmZ)
{
