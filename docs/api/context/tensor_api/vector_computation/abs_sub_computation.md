...
AbsSub计算对两个源 Tensor 的对应元素执行逐元素绝对差值运算，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::AbsSub` 操作类型来实现。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，AbsSub 计算取 `Inst::AbsSub`。 |
| TraitType | 计算特征参数类型，AbsSub 计算使用默认值 `Std::ignore_t`。 |
...
__aicore__ inline void AbsSubComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
    ...
    Transform<Inst::AbsSub>(zLocal, xLocal, yLocal);
