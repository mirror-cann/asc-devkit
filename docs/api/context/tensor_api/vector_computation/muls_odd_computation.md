...
MulsOdd计算对源 Tensor 的奇数位元素与标量值执行乘法，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::MulsOdd` 操作类型来实现。

计算公式为：`dst[2i+1] = src[2i+1] * value`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，MulsOdd 计算取 `Inst::MulsOdd`。 |
| TraitType | 计算特征参数类型，MulsOdd 计算使用默认值 `Std::ignore_t`。 |
...
- `Inst::MulsOdd` 仅处理奇数位元素，通常与 `Inst::MulsEven` 配合使用以覆盖完整 Tensor。
...
__aicore__ inline void MulsOddComputeExample(__gm__ half* gmX, __gm__ half* gmZ)
{
    ...
    Transform<Inst::MulsOdd>(zLocal, xLocal, half(2.0));
