...
代码实现路径为：`impl/tensor_api/arch/vector/binary_scalar`。

ShiftRightScalar计算对源 Tensor 的每个元素与标量值执行逻辑右移，将结果写入目的 Tensor。通过 `Transform` 接口传入 `Inst::ShiftRightScalar` 操作类型来实现。

计算公式为：`dst[i] = src[i] >> value`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，ShiftRightScalar 计算取 `Inst::ShiftRightScalar`。 |
| TraitType | 计算特征参数类型，ShiftRightScalar 计算使用默认值 `Std::ignore_t`。 |
| DstTensor | 目的张量类型，需为 Tensor API Tensor 类型，存储位置为 `Location::UB`。 |
| SrcTensor | 源张量类型，需为 Tensor API Tensor 类型，存储位置为 `Location::UB`。 |
| ScalarType | 标量值类型，表示移位位数。 |
...
| src | 输入 | 被移位源张量，存储位置为 `Location::UB`。 |
| value | 输入 | 标量移位位数。 |
...
| 操作类型 | 支持的数据类型 |
| :--- | :--- |
| `Inst::ShiftRightScalar` | uint8_t, int8_t, uint16_t, int16_t, half, int32_t, uint32_t, float |
...
__aicore__ inline void ShiftRightScalarComputeExample(__gm__ uint16_t* gmX, __gm__ uint16_t* gmZ)
{
    ...
    Transform<Inst::ShiftRightScalar>(zLocal, xLocal, uint16_t(2));
