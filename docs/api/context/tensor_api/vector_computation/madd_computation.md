...
Madd计算对两个源 Tensor 的对应元素执行逐元素乘加，并将结果累加到目的 Tensor 原有值上。通过 `Transform` 接口传入 `Inst::Madd` 操作类型来实现。

计算公式为：`dst[i] = src0[i] * src1[i] + dst[i]`。
...
| 参数名 | 描述 |
| :--- | :--- |
| CalcFunc | 计算操作类型，Madd 计算取 `Inst::Madd`。 |
| TraitType | 计算特征参数类型，Madd 计算使用默认值 `Std::ignore_t`。 |
| DstTensor | 目的张量类型，需为 Tensor API Tensor 类型，存储位置为 `Location::UB`，同时作为累加输入和结果输出。 |
...
| 参数名 | 输入/输出 | 描述 |
| :--- | :---: | :--- |
| dst | 输入/输出 | 目的张量，存储位置为 `Location::UB`。既作为累加输入，也作为结果输出。 |
...
- `Inst::Madd` 会读取目的 Tensor 原有值，调用前需保证目的 Tensor 已完成初始化或已搬入有效数据。
...
__aicore__ inline void MaddComputeExample(__gm__ half* gmX, __gm__ half* gmY, __gm__ half* gmZ)
{
    ...
    Transform<Inst::Madd>(zLocal, xLocal, yLocal);
