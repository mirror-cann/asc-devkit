...
L1 Buffer到Fixpipe Buffer搬运接口是在built-in接口`asc_copy_l12fb`的基础上进行抽象封装实现的，其对应的底层built-in接口为：

```cpp
__aicore__ inline void asc_copy_l12fb(__fbuf__ void* dst, __cbuf__ void* src, uint16_t burst_num,
