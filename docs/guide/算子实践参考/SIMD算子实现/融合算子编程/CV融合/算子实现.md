    ...
    __gm__ uint8_t* c, __kfc_workspace__ __gm__ uint8_t* workspace, AscendC::tiling::TCubeTiling tiling)
{
    ...
    while (matmulObj.template Iterate<true>())
    {
        ...
        // CopyOut逻辑：将融合计算结果搬出到GM，并更新computeRound;
        computeRound++;
    }
    matmulObj.End();
}
...
2.  **设置A、B、Bias的数据类型和格式。**
    ...
    设置示例如下，关于TPosition的详细内容请参考[TPosition](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/其他数据类型/TPosition.md)。
...
>        ```
>    - 若算子工程不是[自定义算子工程](../../../../编程指南/高级编程/高级特性/Aclnn算子工程化开发/概述.md)，也不是带有[HAVE\_WORKSPACE](../../../../编程指南/附录/基于样例工程完成Kernel直调.md#table481718169817)编译宏的Kernel直调算子工程，kernel侧需要在Matmul初始化前，通过[SetSysWorkSpace](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/基础API/资源管理/临时空间管理/workspace/SetSysWorkSpace.md)设置系统workspace。
>        ```
...
>    } 
>    ```
>    完整样例请参考[BareMix样例](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/03_libraries/00_matrix/matmul_fused_mannul)。
