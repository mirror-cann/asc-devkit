...
A、B、C矩阵的Layout类型都为NORMAL时，不支持[batchMode](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/高阶API/矩阵计算/Matmul-Kernel侧接口/MatmulConfig.md)参数设为SINGLE\_LARGE\_THAN\_L1，即Bias复用场景下，单Batch的A、B矩阵数据总和不得超过L1 Buffer的大小。
