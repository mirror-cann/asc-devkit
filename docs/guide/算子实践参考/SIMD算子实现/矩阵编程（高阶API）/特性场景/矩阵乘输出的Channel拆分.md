...
由于1个float类型数据的大小为4字节，16\*8的分形在内轴满足32字节对齐，内轴上的数据量与一条NPU矢量计算指令处理的数据单元一致，这便于后续的其它计算。ChannelSplit功能默认不启用，用户需通过设置[MatmulConfig](https://gitcode.com/cann/asc-devkit/blob/9.1.0/docs/api/SIMD-API/高阶API/矩阵计算/Matmul-Kernel侧接口/MatmulConfig.md)中的isEnableChannelSplit参数为true来开启此功能。
...
完整的算子样例请参考[matmul\_channelsplit算子样例](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/01_simd_cpp_api/04_advanced_api/00_matmul/matmul_channelsplit_output)。
