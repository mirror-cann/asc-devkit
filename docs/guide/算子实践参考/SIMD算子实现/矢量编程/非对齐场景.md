...
DataCopyPad接口提供非对齐搬运的功能，如果基于该接口支持的产品开发算子（参见产品支持情况），则可以直接使用该接口解决非对齐场景下的搬运问题。使用DataCopyPad的完整示例请参考[DataCopyPad样例](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.3/examples/01_simd_cpp_api/02_features/03_basic_api/00_data_movement/data_copy_pad_gm2ub_ub2gm)。
...
-   **通过Duplicate逐行清零**。计算前，针对每一行数据，调用基础API [Duplicate](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta3/API/ascendcopapi/atlasascendc_api_07_0088.html)对冗余数据位置填充0值。
-   **通过Pad一次性清零**。计算前，针对多行数据，可以采用高阶API [Pad](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta3/API/ascendcopapi/atlasascendc_api_07_0849.html)接口对冗余数据一次性清零。

由于搬出时搬运的数据长度和操作数的起始地址（UB上）必须保证32字节对齐，搬出时可以选择去除冗余数据或者带着冗余数据搬出的方式。

-   **使用UnPad接口去除冗余数据后搬出**。待搬出的有效数据总长度满足32字节对齐时，可使用高阶API [UnPad](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta3/API/ascendcopapi/atlasascendc_api_07_0851.html)接口去除冗余数据并完整搬出。
-   **使用GatherMask收集有效数据后搬出**。待搬出的有效数据总长度大于等于32字节时，可使用[GatherMask](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta3/API/ascendcopapi/atlasascendc_api_07_0071.html)重新收集有效数据，保证搬出的有效数据起始地址和数据长度32字节对齐。
-   **带冗余数据搬出**。注意多核处理时开启原子加（使用[SetAtomicAdd](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta3/API/ascendcopapi/atlasascendc_api_07_0210.html)接口），防止数据踩踏。
...
-   样例一：[冗余数据参与计算+使用GatherMask收集有效数据后搬出](https://gitcode.com/cann/asc-devkit/blob/9.1.0-beta.3/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_gather_mask)。
...
-   <a name="zh-cn_topic_0000001945665465_li1891184254913"></a>样例二：[通过Duplicate逐行清零+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.3/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_duplicate)。
...
-   样例三：[冗余数据参与计算+使用UnPad接口去除冗余数据后搬出](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.3/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_unpad)。
...
-   示例四：[通过Pad一次性清零+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.3/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_pad)。
...
-   样例五：[使用mask掩掉冗余数据+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.3/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/reduce_min)。
