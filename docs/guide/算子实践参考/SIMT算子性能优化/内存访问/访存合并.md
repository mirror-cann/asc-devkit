...
- 优化数据切分实现访存合并
  ...
  【样例介绍】以Gather算子为例，算子按行从输入中取数，计算公式可以表示为`output[row, col] = input[index[row], col]`。输出矩阵每一行内部的列方向数据在GM上连续，优化切分时应让同一个Warp内相邻线程尽量访问同一行内的相邻列，而不是只保证单个线程内部连续。
...
- 引入UB中转实现访存合并
  ...
  【样例介绍】以矩阵转置为例，输入矩阵shape为1024 x 1024，数据类型为`float`。矩阵转置先将矩阵划分为多个固定大小的子矩阵块进行处理，这里的子矩阵块称为tile。本样例采用32 x 32的tile，每个ThreadBlock负责处理一个tile。Grid配置为`(matrix_width / 32, matrix_height / 32, 1)`，ThreadBlock配置为`(32, 32, 1)`，每个thread处理tile中的1个元素。该切分方式下，`threadIdx.x`方向的32个thread对应tile内同一行的32个连续元素，适合形成连续GM读访问。
