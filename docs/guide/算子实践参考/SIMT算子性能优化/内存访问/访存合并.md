...
- 引入UB中转实现访存合并
  ...
  上述实现先让每个thread按原始布局从GM读取数据，并写入UB中的 `tile[threadIdx.y][threadIdx.x]`。同步后，交换ThreadBlock坐标定位到转置后的输出tile，再从UB中读取 `tile[threadIdx.x][threadIdx.y]`并写回GM。这样处理后，同一个Warp在GM侧读取输入的一行，也写回输出的一行，GM读写都保持连续；转置造成的非连续访问被转移到UB内部完成。
