...
避免寄存器溢出的思路是：先通过`--cce-res-usage`编译选项查看核函数的寄存器使用情况（`Stack size`大于0即表明存在溢出），再根据上表中寄存器与最大线程数的对应关系，选择一档能满足算子单线程寄存器需求的最大线程数，并通过`__launch_bounds__(N)`将其配置给编译器。编译器据此放宽每线程的寄存器配额，从而避免寄存器溢出，将中间数据保留在寄存器中，减少Global Memory访问，提升性能。一般建议：计算密集型算子（如sincos）配置512或1024线程，数据搬运类算子配置2048线程。关于`__launch_bounds__`的详细说明，请参考[SIMT BuiltIn关键字](../../../编程指南/语言扩展层/SIMT-BuiltIn关键字.md#核函数配置)。
