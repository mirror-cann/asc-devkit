...
避免同地址访问的方法主要是**调整数据访问顺序**。下文介绍配套的样例请参考[DataCopy最佳实践样例](https://gitcode.com/cann/asc-devkit/blob/9.1.0/examples/01_simd_cpp_api/05_best_practices/04_memory_access_practices/data_copy)中的优化点4。
...
for (uint32_t mBlockIdx = 0;
     mBlockIdx &lt; fullMBlockCount;
     mBlockIdx++) {
    // 优化实现：在每组 numBlocks 个 M 块内，
    // 按当前核的 blockIdx 轮转访问顺序。
