...
>[!NOTE]说明 
>下文仅列出代码片段，完整样例请参考[动态输入算子样例](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.2/examples/01_simd_cpp_api/02_features/03_basic_api/04_resource_management/list_tensor_desc_input)。

-   kernel直调
    -   参考[ListTensorDesc](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta2/API/ascendcopapi/atlasascendc_api_07_0009.html)数据结构自行定义ListTensorDesc和TensorDesc结构体，并将实际的输入数据保存至ListTensorDesc结构中。示例如下:
    ...
    -   kernel侧调用时，直接传入ListTensorDesc表达的输入信息。示例如下：
        ...
        ```
            ...
            list_tensor_desc_input_custom<<<numBlocks, nullptr, stream>>>(inputDescInDevice, zDevice, tiling);
        ```
