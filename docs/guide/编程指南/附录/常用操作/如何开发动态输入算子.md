...
>[!NOTE]说明 
>下文仅列出代码片段，完整样例请参考[动态输入算子样例](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/01_basic_api/04_memory_management/list_tensor_desc_input)。

-   kernel直调
    -   参考[ListTensorDesc](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/其他数据类型/ListTensorDesc.md)数据结构自行定义ListTensorDesc和TensorDesc结构体，并将实际的输入数据保存至ListTensorDesc结构中。示例如下:
