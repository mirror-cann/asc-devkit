...
-   blocks\_per\_grid：int或[dim3](https://gitcode.com/cann/asc-devkit/blob/master/docs/guide/编程指南/语言扩展层/SIMT-BuiltIn关键字.md#内置结构体)类型，用于指定网格（grid）的维度与规模，blocks_per_grid.x * gblocks_per_grid.y * blocks_per_grid.z等于启动的线程块总数，不得大于65535。
...
-   blockDim：获取线程块中配置的三维层次结构，即启动VF时配置的dim3结构体实例值。blockDim.x，blockDim.y，blockDim.z分别裴示线程块中三个维度的线程数。在内核启动的执行配置中指定，对应核函数配置中的threads_per_block参数。
