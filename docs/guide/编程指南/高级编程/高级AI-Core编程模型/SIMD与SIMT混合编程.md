...
-   核函数定义方式
    -   SIMT VF函数定义：
        ...
        SIMD与SIMT混合编程中定义SIMT VF函数需要使用[\_\_simt\_vf\_\_](../../语言扩展层/SIMD-BuiltIn关键字.md#li611618392141)函数标记宏标识。SIMT VF函数支持指针传参，指针变量必须用\_\_gm\_\_或\_\_ubuf\_\_地址空间限定符标识。关于SIMT VF函数编程的相关约束请参考[函数约束](../../../技术附录/CPP标准支持/语法限制/函数.md)。
...
样例中介绍的算子完整代码请参见[SIMD与SIMT混合编程实现gather&adds算子样例](https://gitcode.com/cann/asc-devkit/tree/9.1.0-beta.2/examples/01_simd_cpp_api/00_introduction/03_fusion_operation/gather_adds_simt_simd_hybrid)。
