...
-   在SIMT VF执行空间读GM时，可能存在缓存数据不一致的问题：
    ...
    -   读数据时，若访问的内存在SIMT DCache中命中，默认会读取Cache数据，可能导致读取到的数据与GM中的最新数据不一致。可使用`volatile`关键字获取最新数据，或使用[asc\_dcci\_single](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMT-API/访存函数/asc_dcci_single.md)接口刷新指定地址所在的Cache Line，使用[asc\_dcci\_entire](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMT-API/访存函数/asc_dcci_entire.md)接口刷新核内整个Data Cache。
