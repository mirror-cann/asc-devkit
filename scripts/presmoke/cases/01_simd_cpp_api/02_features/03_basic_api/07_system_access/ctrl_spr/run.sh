...
CASE_REL=01_simd_cpp_api/02_features/03_basic_api/07_system_access/ctrl_spr
...
case_verify() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'python3 ../scripts/verify_result.py')
}
