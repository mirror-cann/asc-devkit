...
CASE_REL=01_simd_cpp_api/02_features/03_basic_api/01_matrix_compute/mmad_gemv
...
case_build() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'cmake -DDISABLE_GEMV=0 .. -DCMAKE_ASC_ARCHITECTURES="$ARCH" $RUN_MODE_ARG')
