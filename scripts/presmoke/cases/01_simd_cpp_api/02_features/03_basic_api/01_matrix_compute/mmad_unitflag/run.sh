...
CASE_REL=01_simd_cpp_api/02_features/03_basic_api/01_matrix_compute/mmad_unitflag
...
case_build() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'cmake -DENABLE_UNITFLAG=1 .. -DCMAKE_ASC_ARCHITECTURES="$ARCH" $RUN_MODE_ARG')
