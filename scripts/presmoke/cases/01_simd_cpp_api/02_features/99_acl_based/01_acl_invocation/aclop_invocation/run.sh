...
CASE_REL=01_simd_cpp_api/02_features/99_acl_based/01_acl_invocation/aclop_invocation
...
case_build() {
    ...
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'atc --singleop=../add_custom.json --output=. --soc_version=${soc_version}')
