...
case_build() {
    ...
    (cd "$BUILD_DIR" && SCENARIO_NUM=1 soc_version=$SOC_VERSION bash -lc 'python3 ../scripts/gen_data.py -scenario_num=1')
}
...
case_verify() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && SCENARIO_NUM=1 soc_version=$SOC_VERSION bash -lc 'python3 ../scripts/verify_result.py ./output/output.bin ./output/golden.bin -scenario_num=1')
}
