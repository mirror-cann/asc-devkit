...
CASE_REL=01_simd_cpp_api/04_advanced_api/00_matmul/matmul_a2b2_share
...
case_verify() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'python3 ../scripts/verify_result.py output/output1.bin output/golden1.bin output/output2.bin output/golden2.bin')
}
