...
CASE_REL=01_simd_cpp_api/04_best_practices/00_vector_compute_practices/add_high_performance
...
case_build() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'cmake -DSCENARIO_NUM=6 .. -DCMAKE_ASC_ARCHITECTURES="$ARCH" $RUN_MODE_ARG')
