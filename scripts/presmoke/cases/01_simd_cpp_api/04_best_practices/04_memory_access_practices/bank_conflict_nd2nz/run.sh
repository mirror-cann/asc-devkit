...
CASE_REL=01_simd_cpp_api/04_best_practices/04_memory_access_practices/bank_conflict_nd2nz
...
case_build() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'cmake -DSCENARIO_NUM=1 .. -DCMAKE_ASC_ARCHITECTURES="$ARCH" $RUN_MODE_ARG')
