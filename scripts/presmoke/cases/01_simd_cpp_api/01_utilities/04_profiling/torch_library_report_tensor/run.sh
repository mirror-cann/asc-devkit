...
CASE_REL=01_simd_cpp_api/01_utilities/04_profiling/torch_library_report_tensor
...
case_run() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc 'msprof --application="python3 ../torch_library_report_tensor.py" --output="../result"')
}
