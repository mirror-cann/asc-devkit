...
CASE_REL=03_simt_api/00_introduction/01_gather/general_gather
...
case_run() {
    mkdir -p "$BUILD_DIR"
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc ./gather)
    (cd "$BUILD_DIR" && soc_version=$SOC_VERSION bash -lc ./gather_v2)
}
